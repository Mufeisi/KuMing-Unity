...
| 8-11 | G8 收官 + Android 侧 G7 判定 | 8·收官 | ✅(G8 GO 条件) | 8-2~8-10 | 1d |
| 9-1 | PC Player UI 收尾决策（RT 直绘定案，关 C4） | 9 | ✅ | — | 0.5d |
| 9-2 | 安装/补丁/首启/异常恢复流程 | 9 | ✅(核心侧) | 9-1 | 2~3d |
| 9-3 | 8/24/72h 长稳 + 多 GPU/分辨率矩阵 | 9 | ✅(核心侧) | 9-1 | 2~3d |
| 9-4 | 性能优化（按数据，禁止无数据先优化） | 9 | 🔲 | 9-3 | 3~5d |
| 9-5 | 灰度发布准备 + G6 判定 | 9 | ✅(有条件) | 9-2~9-4 | 2~3d |
...
状态：✅（2026-08-08，commit 22042ac，`Build/manifestverify.ps1` PASS cases=4）｜预估：1d｜依赖：阶段7 项4 已备
...
#### 8-9-2 OTA：资源下载系统
状态：✅（2026-08-08，commit f0d1347，manifestverify PASS cases=5；真机 E2E 首轮 FAIL 已修复留阶段收口）｜预估：1d｜依赖：8-9-1

- 目标：首启下载 + 进度 + 校验落盘（替代 adb push）。
- 开发：`ResourceSync.DownloadFile` 接入 MobileBootstrap 首启；进度/失败重试。
- 测试：resourcesyncverify 扩展真机场景。
- 验证：探针 PASS + 真机冷启动下载进图（E2E 留收口）。
- 提交：`feat(阶段8): OTA 下载系统`。
- 验收：真机卸载重装后自动下载进图。

#### 8-9-3 OTA：增量更新
状态：✅（2026-08-08，commit d5612fc，manifestverify PASS cases=6）｜预估：1d｜依赖：8-9-2
...
#### 8-9-4 OTA：断点续传/失败恢复
状态：✅（2026-08-08，commit f6e5a48，manifestverify PASS cases=7；真机弱网留阶段收口）｜预估：1d｜依赖：8-9-2
...
#### 8-10 性能分级动态降级落地
状态：✅（2026-08-08，commit 3e514e0，devicetierverify PASS cases=3；低端真机采样留阶段收口）｜预估：2~3d｜依赖：8-9-2
...
状态：✅（2026-08-08，G8 有条件 GO 加权 81.1%；G7-Android 待真机判定）｜预估：1d｜依赖：8-2~8-10
...
状态：✅（2026-08-08，ADR：RT 直绘定案关 C4）｜预估：0.5d｜依赖：—
...
#### 9-2 安装/补丁/首启/异常恢复
状态：✅（2026-08-08，commit 373279c，pcstartupverify 2/2；装机验收留阶段收口）｜预估：2~3d｜依赖：9-1
...
#### 9-3 长稳 + 多 GPU/分辨率矩阵
状态：✅（2026-08-08，commit 50cf9b4，pcresolution 3/3 + pcstability 15min PASS；72h/多 GPU 留收口）｜预估：2~3d｜依赖：9-1
...
#### 9-5 灰度发布 + G6 判定
状态：✅（2026-08-08，G6 有条件 GO：P0/P1 清零 + 灰度机制就绪；玩家灰度外部依赖；tag stage9-pc-rc-v1）｜预估：2~3d｜依赖：9-2~9-4
