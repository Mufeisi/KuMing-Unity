# Iteration 1 orchestration: start server (Server.exe standalone) -> Unity UI probe -> assert -> stop server.
# UI probe: login -> StartGame -> real MainDialog+ChatDialog control tree render (S.UserInformation-driven
#   HUD labels + 4 chat types with coloured back rects) -> UiText R8 glyph pipeline (WarmTree pre-build before
#   batch) -> PNG + data/pixel assertions.
param(
...
& $unity -batchmode -projectPath $proj -executeMethod Crystal.Rendering.Editor.NetProbe.RunUi -logFile $log | Out-Null
$code = $LASTEXITCODE
...
$ok = ($code -eq 0) -and ($line -match "ui ok")
