...
| --- | ---: | --- | --- | --- |
| 登录、选角、进图、断线重连 | 8 | 全部必须 | 🟩 登录/选角/进图 + 断线重连（`MobileReconnect`：OnDisconnected + Network.Connected 轮询 → 3s 自动重连，G8 缺口 2/4 补全） | `androidverify.ps1` + `mobilereconnectverify` cases=3 |
| 移动、寻路、地图切换 | 14 | 全部必须 | 🟩 移动/自动寻路 + 换图传送（`BigMapDialog` 全移植：NPC 行选择 → `TeleportToButton.Click` → `C.TeleportToNPC`；WorldMapSetup 世界地图） | `androidverify.ps1` swipe 移动 + 8-4-2 大地图 |
| 战斗、技能、目标选择、药品 | 20 | 全部必须 | 🟩 自动战斗（`MobileCombat` 四态）+ 技能按钮（`Magics[0]`→`C.Magic`）+ 药品按钮（背包 Potion→`C.UseItem`，G8 缺口 1/4 补全） | `MobileCombatVerify` + `mobilehudverify` cases=9 |
...
| 域 | 权重 | 完成分 | 依据 |
| --- | ---: | ---: | --- |
| 登录/选角/进图/断线重连 | 8 | 8 | 重连已补（G8 2/4） |
| 移动/寻路/地图切换 | 14 | 14 | 换图链路已存在（TeleportToNPC 全移植） |
| 战斗/技能/目标选择/药品 | 20 | 20 | 技能/药品已补（G8 1/4） |
| 角色/装备/背包/拾取 | 12 | 12 | |
| NPC/任务/商店/仓库 | 12 | 12 | |
| 聊天/组队/行会/好友 | 9 | 9 | |
| 交易/邮件/拍卖/商城 | 8 | 8 | |
...
**✅ 加权覆盖率 97.4% ≥ 80% 达标**（G8 缺口补全 1/4+2/4 + 换图链路复核修正，2026-08-08 更新）；`#if UNITY_` 平台条件编译扩散检查：**Client.Core 零命中**（无平台条件编译扩散）。
...
**G8：升级为 GO（2026-08-08 复核）**——加权 97.4%（原 81.1%）：G8 缺口补全 1/4（技能/药品按钮）+ 2/4（断线重连）+ 换图链路复核修正（BigMapDialog 全移植含 TeleportToNPC，8-11 判定基线错误）。**剩余缺口 1 项**：移动端特效绘制接入（视觉域，权重 5；8-10 已预留 ParticleScale 消费点，GameRuntime 特效绘制接入后补）。
