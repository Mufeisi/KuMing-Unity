# Iteration 6 orchestration: start server (Server.exe standalone) -> Unity team probe -> assert -> stop server.
# Team probe: login -> StartGame -> render real GroupDialog/FriendDialog/GuildDialog control tree
#   (group member list / friend online-green + offline-white rows / guild status page + notice) -> PNG + assertions.
param(
...
& $unity -batchmode -projectPath $proj -executeMethod Crystal.Rendering.Editor.NetProbe.RunTeam -logFile $log | Out-Null
$code = $LASTEXITCODE
...
$ok = ($code -eq 0) -and ($line -match "team ok")
