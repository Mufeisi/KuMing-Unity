    ...
    public sealed class MobileBootstrap : MonoBehaviour
    {
        ...
        void PollJoystick()
        {
            ...
            for (int i = 0; i < Input.touchCount; i++)
            {
                ...
                {
                    UiConsumer = (id, ph, ui) => _bag.OnTouch(id, ph, ui) || _equip.OnTouch(id, ph, ui) || _quest.OnTouch(id, ph, ui) || _map.OnTouch(id, ph, ui) || _chat.OnTouch(id, ph, ui) || _group.OnTouch(id, ph, ui) || _friend.OnTouch(id, ph, ui) || _guild.OnTouch(id, ph, ui) || _mail.OnTouch(id, ph, ui) || _market.OnTouch(id, ph, ui) || _gameShop.OnTouch(id, ph, ui) || _hero.OnTouch(id, ph, ui), // 背包/装备/任务/地图/聊天/组队/好友/行会/邮件/拍卖行/商城/英雄按钮（ui 空间，短路：背包先消费）
                    PanelOpen = bagOpen,
