...
{
    // 逐字移植（2026-08-06）：Client/MirScenes/Dialogs/KeyboardLayoutDialog.cs
    // 按键绑定设置对话框：Title 119 框体，滚动行列表（分组标题 + 绑定行）+ 重置/严格规则按钮 + 滚动条。
    // 依赖迭代包10 InputKeys seam：CMain.InputKeys（Keylist/DefaultKeylist/Save/GetKey）+ 扩充后的 Keys 枚举
    // （字母/数字/NumPad/修饰键/Oem8）。WaitingForBind 由渲染层键盘钩子 CheckNewInput 落定，探针直接驱动。
