...
**进行中：阶段8 第7项 已全收口（8-7-1 交易 ✅、8-7-2 邮件 ✅、8-7-3 拍卖行 ✅）；下一增量 8-7-4 商城。阶段8 第6项 已全收口（8-6-1 组队 ✅、8-6-2 好友 ✅、8-6-3 行会 ✅）。阶段8 第4/5项 已全收口（8-4-1 任务四窗 ✅、8-4-2 大地图 ✅、8-4-3 小地图 ✅、8-5-1 软键盘+安全区 ✅、8-5-2 聊天 ✅）。阶段8 第2项 背包与物品已全收口（8-2-1..8-2-5 ✅），第3项 NPC/商店/仓库 已全收口（8-3-1..8-3-3 ✅）**

- **8-7-3 完成（2026-08-08）**：拍卖行触控收口。`TrustMerchantDialog` 竞拍出价分支接回（对话框本体交互此前随 PC 迭代包7 已移植，唯一缺口）：`BuyButton` Auction case 弹 `MirAmountBox`（旧客户端逐字：min=当前价+1、default=当前价+1）→ OK 发 `C.MarketBuy{AuctionID, BidPrice}`（3000ms 节流同款；**无 MirMessageBox 二次确认**，对齐 UserMode/Consign/GameShop 直发包惯例——旧版弹确认框依赖未移植弹窗基类）。`GameSession` 补 5 派发（`internal static` + `InternalsVisibleTo` 供探针）：`S.NPCMarket`（Bind 逐件 + `dlg.Show()`（内部 TMerchantDialog(Market) 发 C.MarketSearch 拉首页+开背包，对齐 NPC 入口语义）+ UserMode + Listings 整表 + Page=0 + **PageCount=p.Pages**（服务器算好页数，非本地算）+ UpdateInterface）/`S.NPCMarketPage`（`Visible` 守卫 + Bind + 追加 + `Page=(Count-1)/10`）/`S.ConsignItem`（`InventoryDialog.GetCell` 背包格解锁，成功清格 + `MapObject.User.RefreshStats`；**Unity 无 BeltDialog** → 仅背包，Belt 分支裁剪）/`S.MarketFail`（Reason 0-10 全映射中文提示 + `MarketTime=0` 节流归零）/`S.MarketSuccess`（`MirMessageBox.Show(p.Message)` + 节流归零）。`GameScene` 挂 `TrustMerchantDialog` 字段（TradeDialog 旁）；`InitInGameDialogs` 常驻创建默认隐藏（顺序契约：inv 已建后——Show 移背包右上）。`MobileBootstrap` 市场按钮（第 9 列 `(ButtonH+8)*8`，橙 tint，mail 下方）+ `ToggleMarket`（开→`Show()` 发 C.MarketSearch+开背包+互斥关装备/组队/好友/行会/邮件列表+Cancel 摇杆/HUD/拾取/寻路；**背包保持开**——市场选物寄售需要，对齐商店同款；关→`Hide()`（清 Listings+解寄售源格锁+背包归位））+ UiConsumer 链 + BackHandler（mail 之后 chr 之前）+ uiOpen/bagOpen 守卫追加 + RenderHud 每帧 Draw+WarmTree。`ProbeLang` 补 `BidAmount` 中文。`FilterButtons` 改 public（探针断言 C.MarketSearch 参数；Filter.Index 是筛选身份非按钮 Index——按钮 Index 恒 920 图片索引）。验证：`Build/marketverify.ps1` → `[marketverify] PASS cases=20`（常驻隐藏/整表 10 件+背包连带+PageCount=Pages/21 件 3 页/翻页追加 Page=1/面板关守卫不追加/Hidden 后 Hide 清空+拦截/FindButton Match 透传/筛选树 weapon Type+Usermode=false/Refresh 发包/Consign 直发 BidPrice=0/Auction 弹框+输入 500+OK 发包 BidPrice=500+框关/节流拦截不发+不弹框/UserMode 取回 AuctionID/SellNow/Sold 取回+Refresh/寄售链路 ItemCell_Click→SellItemSlot→Price=100→C.ConsignItem{Type=Consign}/回声成功解锁清格/失败仅解锁保格/MarketFail 弹提示+节流归零/MarketSuccess/四面板切换/RouteTouch 按钮消费开关面板+不喂摇杆+开面板发 C.MarketSearch）+ 回归 gamesessionverify PASS + mailverify 16/16 + mobileuiverify 7/7 + `net-market.ps1`（RenderMarket 交易+邮件+委托渲染像素断言）PASS + CoreVerify 0 错误 + build.ps1 8/8。**探针踩坑**：`S.NPCMarket.Auctions` 字段实为 `Listings`（+自带 `Pages`，探针须显式填）→ 修正派发与探针；`TrustMerchantDialog.UserMode`/`Selected`/`SellItemSlot` 为 static → 探针实例引用 `dlg.UserMode=` 触发 CS0176，夹具复位 + 静态引用；筛选按钮 `b.Index != 0` 过滤无效（按钮 Index 恒 920）→ 改固定顺序 `FilterButtons[1]`=weapon；`dlg.Hide()` 清 Listings（对话框自身语义）→ case5 断言 Count==0（Hide 清空+守卫拦截双重验证）；`RefreshStats` 依赖 `scene.BuffsDialog`（RefreshBuffs 空引用 NRE，UseItemVerify 同款踩坑）→ NewScene 补 `new BuffDialog()`；`MirAmountBox.OKButton.Click` 内置 Dispose 先注册先执行，追加 handler 读 Amount 仍有效。**下一步**：8-7-4 商城触控化（GameShopDialog，PC 迭代包9）。

- **8-7-1 完成（2026-08-08）**：交易窗口触控收口。`TradeDialog`/`GuestTradeDialog`（PC 迭代包7 已移植）网络交互接回 + 常驻 + 入口。`GameSession` 补 8 派发（`internal static` + `InternalsVisibleTo` 供探针，旧客户端 GameScene 逐字对齐）：`S.TradeRequest`（被邀弹 `MirMessageBox` YesNo，`PlayerRequestedTrade` 本地化 `"玩家 {0} 要求与你交易"`；Yes→`C.TradeReply{AcceptInvite=true}`，No→`{false}`）/`S.TradeAccept`（记 `GuestName` + `TradeDialog.TradeAccept()` 移背包右上 + Show 双方）/`S.TradeGold`（`GuestGold` 记数 + `ChangeLockState(false)` 解锁 + `RefreshInterface`）/`S.TradeItem`（`GuestTradeDialog.GuestItems` 整表 + 解锁 + 刷新）/`S.TradeConfirm`（`TradeReset` 清物品/金币/解锁/隐藏双方）/`S.TradeCancel`（`Unlock=true` 仅解锁；否则 `TradeReset` + `"交易取消\r\n要完成交易必须面对对方"` 提示）/`S.DepositTradeItem`/`S.RetrieveTradeItem`（回声 Success → 本地移物品 + 解锁双格 + 解锁交易；**Unity 无 BeltDialog** → 背包格 `ItemSlot=6+idx`，`From/To < BeltIdx` 跳过腰带格无对应 UI）。`InitInGameDialogs` 常驻创建双方面板（默认隐藏，顺序契约：TradeDialog 先建因 `RefreshInterface` 引 GuestTradeDialog）。**放入/取回两段式触控**（`MirItemCell.OnMouseClick` else 分支 + 新 `TradeTransfer()`）：选中源格（有物品）→ 点异网格空目标格 → 发 `C.DepositTradeItem{From=源 ItemSlot,To=目标}`/`C.RetrieveTradeItem`，双格 `Locked` 防连点 + `ChangeLockState(false)`；同网格点空格清选中（原语义）。**金币输入**：`GoldLabel.Click` 接回 `MirAmountBox`（8-5-1 软键盘桥即用，`TradeAmount` 本地化 `"交易金额:"`）→ OK 且 `Amount>0` → `TradeGoldAmount` 累加 + `C.TradeGold{Amount}`（`SelectedCell==null && Gold>0` 守卫对齐旧客户端）；`ConfirmButton` 锁定/解锁 → `C.TradeConfirm{Locked}`；`CloseButton` → Hide 双方 + `C.TradeCancel`。`Client.Rendering/MobileTrade.cs` 交易入口控制器（仿 `MobileNpc`）：地图 tap → 屏转格 → 最近存活非己方 `PlayerObject`（≤TapRadius=1）命中 → `C.TradeRequest`（3000ms 节流，期内消费不重发；`C.TradeRequest` 无目标字段，服务器按玩家朝向解析）。`MobileBootstrap` 接线：tap 链 `_npc → _trade → _pickup`；`BackHandler` 交易面板优先关（`CloseButton.InvokeMouseClick` 复用完整关闭语义 = Hide 双方 + `C.TradeCancel`）；`uiOpen`/`bagOpen` 守卫追加 `TradeDialog.Visible`；RenderHud 每帧 `trade.Draw()`+`guest.Draw()`+`UiText.WarmTree`（开才预热）。`ProbeLang` 补 `PlayerRequestedTrade`/`TradeAmount` 中文条目（探针渲染断言文本）。验证：`Build/tradeverify.ps1` → `[tradeverify] PASS cases=22`（常驻创建默认隐藏/TradeRequest 弹 YesNo 模态/Yes→C.TradeReply{true}/No→{false}/TradeAccept 双方 Show+背包移右上/TradeGold GuestGold+解锁+金币标签刷新/TradeItem GuestItems+解锁/ConfirmButton 锁定解锁两拍 C.TradeConfirm/S.TradeConfirm TradeReset 清空+隐藏/TradeCancel Unlock 仅解锁/非 Unlock 重置+提示/DepositTradeItem 回声移物品+解锁双格+解锁交易/RetrieveTradeItem 回声/两段式放入 From=6,To=0+双格锁+清选中/两段式取回/GlodLabel MirAmountBox 弹框+max=Gold+输入 123+OK→C.TradeGold{123}+累加/Gold=0 不弹框/MobileTrade 命中玩家发 C.TradeRequest/无玩家拒绝/死亡玩家排除/3000ms 节流消费不重发冷却后重发/CloseButton Hide 双方+C.TradeCancel）+ PC 回归 `net-market.ps1`（NetProbe RenderMarket 交易+邮件+委托渲染像素断言）。**探针踩坑**：`GuestTradeDialog.GuestItems` 为**静态**字段（`guest.GuestItems =` 触发 CS0176 实例引用静态成员）→ `GuestTradeDialog.GuestItems` 直写；`GuestName/GuestGold` 为实例字段随新实例复位，夹具仅重置静态 `GuestItems`；跨 case 静态 `GuestItems` 长度会被 `S.TradeItem` 整表赋值覆盖（`new[]{item}` 长度 1）→ `NewScene` 重置 `new UserItem[10]` 防后续 TradeReset 越界。**下一步**：8-7-2 邮件触控化。

- **8-6-3 完成（2026-08-08）**：行会流程触控收口。`GuildDialog` 网络交互接回（PC 迭代包6 探针留空处补全）：Show 未加入行会（`string.IsNullOrEmpty(GuildName)`——`UserObject.GuildName` 默认 null，`== ""` 会把 null 误判为已加入，三处 `== ""` 全改）弹 `MirMessageBox` OK（`NotInGuild`）不打开；已加入 → Visible + `NoticeChanged && LastNoticeRequest<Time` 时发 `C.RequestGuildInfo{Type=0}`（5s 节流，`LastNoticeRequest=Time+5000`）。公告滚动纯 C# 化（`InputTextBox.GetFirstCharIndexFromLine/ScrollToCaret` WinForms 不可用）：`Notice` MirTextBox 保留作数据源（`Visible=false`，NetProbe 直写 `Notice.Text` 契约不变），`TextChanged → SyncFromTextBox` 读回全文；渲染委托 25 行 `NoticeLineLabels` 窗口（每行 13px 于 322×330 框内，TextGlyphBuilder 不折行逐行标签）；`NoticeUpButton/DownButton` 驱动 `NoticeScrollIndex`（下滚末端守卫 `>=max(0,count-25)`）+ `NoticePositionBar` 滚动条位置（289px 轨道，y∈[16,298]）+ `OnMoving` 拖动反算。`GameSession` 补 5 派发（`internal static` + `InternalsVisibleTo` 供探针）：`S.GuildStatus`（同步 `User.GuildName/GuildRankName` + 首次入会标记 `NoticeChanged/MembersChanged` + 填 `Level/Experience/MemberCount/MaxMembers`，空行会名 Hide）/`S.GuildNoticeChange`（`update==-1` → `NoticeChanged=true` 下次 Show 拉整表；否则整表 `NoticeChange` 存全文+归零滚动索引+渲染首屏）/`S.GuildInvite`（弹 `MirMessageBox` YesNo：Yes→`C.GuildInvite{AcceptInvite=true}`，No→`{false}`）/`S.GuildMemberChange`（2 加入→`MemberCount++`+行会频道 `PlayerJoinedGuild`，3 除名/4 离开→`PlayerLeftGuild`）/`S.GuildExpGain`（累加状态页经验）。`InitInGameDialogs` 常驻创建 `GuildDialog`（默认隐藏，好友之后）。`MobileBootstrap` 行会入口（`_guild` 按钮栈，黄 tint，好友下方 `SetMargin` 下移 `(ButtonH+8)*6`）+ `ToggleGuild`（开→互斥隐藏背包/角色/组队/好友面板 + 取消摇杆/HUD/拾取/自动寻路再开面板；关→Hide）+ UiConsumer 链 `_guild.OnTouch` + `BackHandler` 关行会面板 + `uiOpen`/`bagOpen` 守卫追加 `GuildDialog.Visible` + RenderHud 每帧 `guild.Draw()`+`UiText.WarmTree`（含瞬态模态框）。验证：`Build/guildverify.ps1` → `[guildverify] PASS cases=17`（常驻创建默认隐藏/Show 未加入弹 OK 提示+不打开/Show 已加入 Visible+发 RequestGuildInfo{Type=0}/5s 节流不发第二次/S.GuildStatus 同步 User+填状态页/首次入会标记 Notice+Members/空行会名 Hide/整表 30 行存储+滚动索引归零+首屏 25 行窗口/Up-Down 窗口平移+滚动条位置+末端守卫/update=-1 标记 Show 重拉/GuildInvite Yes→接受发包/No→拒绝/加入→MemberCount+++行会频道提示/离开提示计数不变/GuildExpGain 累加/CloseButton Hide/RouteTouch 行会按钮消费不喂摇杆+开面板发拉取）+ 回归 gamesessionverify PASS + chatverify 11/11 + groupverify 16/16 + friendverify 16/16 + mobileuiverify 7/7 + `net-ui.ps1`（NetProbe 行会渲染像素断言 notice/guildName/guildLevel/guildMembers 全过）。**探针踩坑**：case10 节流窗内 Show 重拉被 5s 节流挡 → `CMain.Time` 拨到 20000 越过窗口隔离验证 `NoticeChanged→Show 重拉`（节流本身 case4 单独断言）；`MapControl.User` 为静态，探针引用 `scene.MapControl.User` 会触发 CS0176 实例引用静态成员 → 夹具 `MapControl.User`/`MapObject.User` 指向同一 UserObject，改 `MapObject.User.GuildName` 直写。**下一步**：8-7-1 交易窗口触控化。

- **8-6-2 完成（2026-08-08）**：好友流程触控收口。`FriendDialog` 网络交互接回（PC 迭代包6 探针留空处补全）：Add 弹 `MirInputBox`（好友/黑名单 tab 分流提示文案 `FriendEnterBlockName`/`FriendEnterAddName`）→ OK 发 `C.AddFriend{Name,Blocked=_blockedTab}` + `Dispose`（Esc→Cancel 不发包，MirInputBox 已随 8-6-1 移植即用）；Remove 弹 `MirMessageBox` YesNo 确认 → Yes 发 `C.RemoveFriend{CharacterIndex=SelectedFriend.Index}`；MemoButton 开 `MemoDialog`（预填 `Friend.Memo`），OK 发 `C.AddMemo{CharacterIndex,Memo}`；Whisper 离线守卫（`PlayerNotOnline` 系统消息）后走 `WhisperAction` seam（`Action<string>` 字段——移动端接 `MobileChat.OpenWhisper` 填 `ChatTextBox "/名字 "` + `SetChatText` 聚焦 + `SoftKeyboardBridge.Focus` 弹软键盘；PC 语义直填输入框，Core 不碰 ChatDialog 保持解耦）；Show 发 `C.RefreshFriends` 拉整表。`GameSession` 补 `S.FriendUpdate` 派发（`internal static` + `InternalsVisibleTo` 供探针）：整表回声填 `FriendDialog.Friends`，面板开着 `Update(false)`（clearSelection=false 保选中）重建行；`InitInGameDialogs` 常驻创建 `MemoDialog`+`FriendDialog`（默认隐藏，memo 先建因 FriendDialog.Hide/MemoButton 引 scene.MemoDialog）。`MobileChat` 新增 `OpenWhisper` static helper（与 OpenInput 同模式：填框+聚焦+弹软键盘）。`MobileBootstrap` 好友入口：`_friend` 按钮栈（青 tint，`_equip` 列下移 `(ButtonH+8)*5`）+ `ToggleFriend`（开→注入 `WhisperAction` + 互斥隐藏背包/装备/组队面板 + 取消摇杆/HUD/拾取/自动寻路再开面板；关→Hide）+ UiConsumer 链 `_friend.OnTouch` + `BackHandler` 先关 `MemoDialog` 再关 `FriendDialog` + `uiOpen`/`bagOpen` 守卫追加 `FriendDialog.Visible` + RenderHud 每帧 `friend.Draw()`+`memo.Draw()`+`UiText.WarmTree`（备注浮窗独立子窗 Title 209 开才预热）。**对照决策**：EmailButton（邮件未移植）与 FriendRow hover 备注浮层（CreateMemoLabel 未接驳）沿用裁剪。验证：`Build/friendverify.ps1` → `[friendverify] PASS cases=16`（常驻创建默认隐藏/Show 发 Refresh/好友 tab Add 弹输入框 OK→C.AddFriend 非黑名单+输入框收起/黑名单 tab Add→Blocked=true/回声隐藏填充不炸+可见重建行/行点选中+选中迁移/Remove 确认 Yes→C.RemoveFriend{Index}+弹窗关/未选中守卫无弹窗无发包/Whisper 在线 seam 带名/离线守卫拦 seam/Memo 预填 OK→C.AddMemo+浮窗关/黑名单 tab 过滤双向/12 行翻页 Next-Prev 页标签 1/2↔2/2/Cancel 不发包/Close 面板关+Memo 联动隐藏/RouteTouch 好友按钮消费不喂摇杆+开面板发 Refresh）+ 回归 gamesessionverify PASS + mobileuiverify 7/7。**探针踩坑**：`FriendDialog.Rows` 定长 12 数组——探针误访 `Rows[12]` 越界（IndexOutOfRange，改断言末槽位填充即证未溢出）。**下一步**：8-6-3 行会面板触控化。

- **8-6-1 完成（2026-08-08）**：组队流程触控收口。`Client.Core/Controls/MirInputBox.cs` 移植（逐字，Modal 挂 `GameScene.Scene`，Prguse 660 框 + 标题 + 单行 `MirTextBox` + OK/Cancel；Enter→OK / Esc→Cancel 键盘路由；裁剪 `Program.Form.Controls`+`MirTextBox.DialogChanged` WinForms 原生表单）。`GroupDialog` 网络交互接回（PC 迭代包6 探针留空处补全）：SwitchButton→`C.SwitchGroup{!AllowGroup}`（允许态由回声 `S.SwitchGroup` 更新；客户端侧满队 15 人/非队长守卫走 `ChatDialog.ReceiveChat` 提示，不发包）；Add/Del 弹 `MirInputBox` 输入成员名 → `C.AddMember`/`C.DelMember`（OK 发包 + `Dispose`，Esc→Cancel 取消不发包）；`public AddMember(string)` 直发供键盘（守卫同款）。`GameSession` 补 7 派发（`internal static` + `InternalsVisibleTo` 供探针）：`S.SwitchGroup`（`AllowGroup` 同步 + 关队清列表）/`S.DeleteGroup`（清 `GroupList`/`GroupMembersMap`/`BigMapViewPort.PlayerLocations` + `YouHaveLeftGroup` 提示）/`S.DeleteMember`（列表/字典/雷达三处移除 + `PlayerHasLeftGroup` 提示）/`S.AddMember`（入列去重 + `PlayerHasJoinedGroup` 提示）/`S.GroupMembersMap`（成员地图 upsert）/`S.SendMemberLocation`（封包 `System.Drawing.Point`→`MPoint` 转 `BigMapViewPort.PlayerLocations` 雷达 upsert）/`S.GroupInvite`（弹 `MirMessageBox` YesNo：Yes→`C.GroupInvite{AcceptInvite=true}`+开 GroupDialog，No→`{false}` 拒绝，Esc→No 语义对齐移动端 Back）。`InitInGameDialogs` 常驻创建 `GroupDialog`（默认隐藏，mini 之后）。`MobileBootstrap` 组队入口：`_group` 按钮栈（红 tint，`_equip` 列下移 `(ButtonH+8)*4`）+ `ToggleGroup`（开→关背包/装备/摇杆/HUD/拾取/自动寻路再开面板；关→Hide）+ UiConsumer 链 `_group.OnTouch` + `BackHandler` 顶部先关模态框、再关组队面板 + `uiOpen`/`bagOpen` 守卫追加 `GroupDialog.Visible` + RenderHud 每帧 `group.Draw()`+`UiText.WarmTree`（含瞬态模态框）。**探针踩坑**：`C.SwitchGroup` 客户端取反后允许态靠服务器回声更新——连点两拍须中间插 `GroupSwitch(S.SwitchGroup{true})` 回声，否则两拍都发 true；`MirMath.Vector2` 与 `UnityEngine.Vector2` 歧义 → 显式 `UnityEngine.Vector2`（同 SafeArea/ChatVerify）。验证：`Build/groupverify.ps1` → `[groupverify] PASS cases=16`（常驻创建默认隐藏/切换按钮开-回声-关两拍/Add 弹输入框+输入成员名 OK→C.AddMember+输入框收起/Del 弹窗+OK→C.DelMember/直发+满队 15 人守卫+非队长守卫/S.SwitchGroup 回声同步+关队清列表/DeleteGroup 三清/DeleteMember 三处移除/AddMember 去重/GroupMembersMap upsert/雷达 upsert（Point→MPoint）/GroupInvite Yes→接受+开窗/Esc 拒绝不发开窗/MirInputBox Esc 取消无发包/Enter 提交发包/RouteTouch 组队按钮消费不喂摇杆）。**下一步**：8-6-2 好友流程触控化。

- **8-5-2 完成（2026-08-08）**：聊天触控收口。`InitInGameDialogs` 常驻创建 `ChatDialog`（旧客户端 MainDialog ctor 直接建底部聊天窗，Unity 端此前裁剪从未实例化——`GameScene.ChatDialog` 字段一直空置；ChatDialog ctor 读 `MainDialog.Location`，须在 main 之后，NetProbe 顺序契约同款）。`Client.Rendering/MobileChat.cs` 触控控制器（底部左缘两个程序化按钮，白色方块 tint 上色同 `MobileBag`）：**聊天按钮** → `OpenInput`——首次开注入当前频道前缀（按钮色相已表达频道选择，服务器按 `!`/`@` 文本前缀分频道）+ `SetChatText("")`（SetFocus + Visible=true + 光标尾部）+ `SoftKeyboardBridge.Focus`（弹软键盘，8-5-1 桥即用）；**频道按钮** → 循环 0 附近/1 全员 `!`/2 行会 `@`，开着输入框则 `ApplyChannel` 去旧前缀补新前缀并**重开软键盘**（`Poll` SyncText 以键盘文本覆盖框文本——不重开则前缀被键盘旧文本冲掉，钉死此决策）。发送走软键盘 Enter（`SoftKeyboardBridge` Submitted → `ChatTextBox_KeyPress` → `C.Chat`），**不另设发送按钮**（YAGNI）。接线逻辑集中在 `MobileChat` 静态助手 `OpenInput/ApplyChannel/CloseInput`（探针与运行时共用，DRY）。`MobileBootstrap`：`_chat` 按钮 + UiConsumer 链（`_map` 后，ui 空间）+ `RenderHud` 每帧 `chat.Draw()`（常驻底部，旧客户端 GameScene 每帧 Draw 语义）+ `UiText.WarmTree`（ChatLines 文本批前合帧）+ Back 优先关输入（`CloseInput` 对齐 PC Escape 隐藏清空语义，`ChatTextBox_KeyPress` Escape 同款）。**探针踩坑**：ChatVerify 夹具 `FakeKeyboard.Open` 须设 `Text=初始文本`（对齐真实 TouchScreenKeyboard 初始文本即键盘内容，否则开着切频道后 Poll SyncText 又把框文本冲回旧值）；`MirMath.Vector2` 与 `UnityEngine.Vector2` 歧义（同 SafeAreaVerify）→ 显式 `UnityEngine.Vector2`。验证：`Build/chatverify.ps1` → `[chatverify] PASS cases=11`（常驻创建+输入框默认隐藏/聊天 tap 开输入+聚焦+键盘/频道循环 0→1→2→0/文本回流+Enter 提交 C.Chat/前缀注入发送/开着切频道重写前缀+重开键盘/未开切频道不触碰/按钮区外不消费/Cancel 抑制/CloseInput 幂等/RouteTouch 集成消费不喂摇杆）+ 回归 safeareaverify 7/7 + softkeyboardverify 8/8 + minimapverify 6/6 + mobileuiverify 7/7。**下一步**：8-6-1 组队流程触控化。

- **8-5-1 完成（2026-08-08）**：软键盘桥接 + 安全区适配收口。**安全区**：`Client.Rendering/SafeArea.cs` 单一来源——`Screen.safeArea`（左下原点）→ 四边 inset `(left,top,right,bottom)` px（UI 空间左上原点语义，`top=距上缘/bottom=距下缘`），`Provider` seam 注入（探针 stub 假刘海/Home indicator；batchmode `Screen.safeArea`=全屏 → inset 全 0 不破坏现有布局契约）。消费方一律读它（禁各自硬编码）：`MobileHud` 血条（顶/左 inset 下移内缩，新增 `HpPos` 渲染位置属性）+ 攻击按钮（底/右 inset 上抬内缩，`RecomputeLayout` 重算圆心）；`MobileBag.Recompute` 右上按钮列（右/顶 inset 内缩）+ 派生按钮（装备/任务/地图 `SetMargin` 堆叠）自动继承整列同步偏移；`TouchJoystick` 浮动摇杆锚定=触摸点天然安全区内不改造。**软键盘触控接线**（桥 `SoftKeyboardBridge` 于 sanduan P2 交付 + `SoftKeyboardVerify` 8/8，本项补**运行时缺口**：此前无任何 UI 调 `Focus(box)`）：`MobileUiAdapter.RouteTouch` Down 分支在 `DialogHit` 命中后调新增 `TryFocusTextBox`——递归控件树，命中可见且启用的 `MirTextBox` → `SoftKeyboardBridge.Focus`（Open TouchScreenKeyboard，初始文本/密码/最大长度走框属性），Poll 文本回流 + Enter 提交（`KeyPress(Enter)` 进控件树，8-5-2 聊天同链）+ 解绑；`MirTextBox.OnMouseDown` 已 SetFocus 画光标，本步只补弹键盘。**探针踩坑**：`MirControl.Enabled` getter 为 internal（Client.Rendering 程序集也不可读）→ `TryFocusTextBox` 读 `InputTextBox.Enabled`（public 字段，`OnEnabledChanged` 同步）；`Crystal.Client.Core.MirMath` 亦有 `Vector2`（与 UnityEngine 歧义）→ 探针显式 `UnityEngine.Vector2`。验证：`Build/safeareaverify.ps1` → `[safeareaverify] PASS cases=7`（inset 注入读值/HUD 攻击+血条偏移/背包+派生列继承/inset=0 回归不漂移/聚焦 Open+文本回流+Enter 提交/框外·不可见·禁用不聚焦/RouteTouch Down 聚焦+对话框消费不喂摇杆）+ 回归 mobilehudverify 7/7 + bagverify 8/8 + mobileuiverify 7/7 + softkeyboardverify 8/8。**待真机/模拟器**：软键盘弹收与刘海 inset 实机校准（模拟器无刘海，`Screen.safeArea`=全屏 inset=0，布局恒等；真机走 `Screen.safeArea` 原生值）。**下一步**：8-5-2 聊天触控化（输入框调 `SoftKeyboardBridge.Focus` 即用本项接线）。

- **8-4-3 完成（2026-08-08）**：小地图触控收口。`InitInGameDialogs` 常驻创建 `MiniMapDialog`（HUD 右上角，旧客户端 GameScene ctor 直接建、Visible 默认 true）+ `DuraStatusPanel` seam 占位（旧客户端 DuraStatusDialog 未移植，`MiniMapDialog.Toggle`/档位自适应 `SetSmallMode/SetBigMode` 读其 `Location` → 空 `MirImageControl` 防 NRE）。**档位/大地图按钮零新增代码**：`ToggleButton`(Index 2102-2104)→`Toggle()`（`_fade` 1↔0 + `_bigMode` + SetSmallMode/SetBigMode，Index 2090↔2091）与 `BigMapButton`(2096-2098)→`GameScene.Scene.BigMapDialog.Toggle()` 已随对话框移植，TouchInputAdapter 鼠标链触控自动生效。`MobileBootstrap` 接线：RenderHud 每帧 `mini.Process()`（刷地图名 `map.Title`/坐标 `"{x}, {y}"`）+ `UiText.WarmTree` + `Draw`（放在 main.Draw 之后，背包面板打开小地图仍显示，对齐旧客户端 HUD 常驻语义）；`MobileHud.Hit` 扩展小地图 `DisplayRectangle` 区（点小地图按钮走 MirButton.Click 链，不触发世界 tap/拾取）。**对照决策**：`MiniMap_BeforeDraw` 档位自适应保留（`map.MiniMap==0` 强切小档、`>0 && _bigMode` 校正回大档）；MailButton 闪烁/点击裁剪随 PC 语义（`NewMail` seam 未接则裁剪路径不崩）。验证：`Build/minimapverify.ps1` → `[minimapverify] PASS cases=6`（常驻创建+初始大档 2090+按钮存在+DuraStatusPanel 契约/ToggleButton 双向 2090↔2091/Process 地图名+坐标/BigMapButton 开合大地图/MiniMap=0 强切小档+MiniMap>0 校正大档/大档渲染路径稳定）+ 回归 gamesessionverify PASS + mobilehudverify 7/7 + mobileuiverify 7/7 + pickupverify 6/6（Hit 由拾取 tap 使用）。**探针踩坑**：`MirControl.Visible` getter 依赖 Parent 可见 → 断言前 `Show()`；`Enabled` internal get 不可读 → 行为断言；batchmode 空库 `GetTrueSize=(0,0)` 视口 scale 0 → 大档渲染裁剪不抛即可（不量化像素）。**下一步**：8-5-1 软键盘+安全区。

- **8-4-1 完成（2026-08-08）**：任务四窗触控收口。`GameSession` 新增任务派发：`S.NewQuestInfo`（填 `GameScene.QuestInfoList`，NPCObject.Load 按 `NPCIndex` 关联）/`S.ChangeQuest`（Add/Update/Remove → **双 `User` 引用同步** `CurrentQuests`（`GameScene.User`=日记读取 + `MapObject.User`=追踪读取），`TrackQuest`→`QuestTrackingDialog.AddQuest`，Remove→摘追踪清 `Settings.TrackedQuests` 槽位）/`S.CompleteQuest`（移除）/`S.ShareQuest`（空体不移植）。`NpcResponse` 门控：`MapControl.GetObject(GameScene.NPCID) is NPCObject npc && npc.Quests.Count > 0` 才连带弹 `QuestListDialog`（无任务 NPC 只留对话）。`InitInGameDialogs` 常驻创建四窗（NPCDialog 之后顺序契约：QuestTracking→QuestDiary→QuestList→QuestDetail；QuestListDialog ctor 读 `NPCDialog.Size.Width` 定 Location）。`QuestSingleQuestItem` 触控接线：**原 `e as MouseEventArgs` cast（Unity Click 为 EventHandler 恒 null）致交互全死** → 点行=左键（`DisplayQuestDetails`+选中），追踪切换拆到独立 `_trackButton`（Index 917/918，原右键行为）。`MobileBootstrap` quest 入口 + 四窗守卫（uiOpen/bagOpen 追加、BackHandler 逐层关闭）。**对照决策**：Remove 摘追踪（旧客户端 ChangeQuest 同语义，否则 ID 残留致幽灵追踪）；QuestMessage 滚轮不移植（ScrollUp/DownButton 已有）。验证：`Build/questverify.ps1` → `[questverify] PASS cases=9`（NewQuestInfo 落库+NPC 关联/NpcResponse 门控弹列表+对话保留/无任务不弹/Add 双引用+TrackQuest+Settings 落盘/Add 无追踪只入册/Update/Remove 双引用+Remove 摘追踪/日记分组+点行开详情+行选中态/追踪 toggle+Settings 更新/5 条上限第 6 条不生效）+ 回归 npcverify 6/6 + shopverify 9/9 + storageverify 9/9 + bagverify/baginteract/equip/useitem/pickup/gamesessionverify。**探针踩坑**：batchmode 空库下 `MirImageControl.Size` getter（AutoSize 且 Library 非 null → `GetTrueSize(Index)=(0,0)`）把 `_trackButton` 弄成 0×0 命中区、状态文本测量回退把 `_stateLabel` 撑到 93px 盖住按钮 → 夹具强制显式尺寸复现真机命中区（按钮 16×14、状态标签 30×16）。**下一步**：8-4-2 大地图触控化。

- **8-4-2 完成（2026-08-08）**：大地图触控收口。`GameSession` 新增派发：`S.NewMapInfo`（`CreateBigMapButtons` 建 `BigMapRecord`——移动按钮 `Parent=ViewPort`、MouseEnter 坐标标签、Click→`SetTargetMap(目的地)`；NPC 行 `BigMapNPCRow` `Parent=dlg`；幂等 `GameScene.MapInfoList[mapIndex]=record`，旧客户端 `Add` 重复 key 会抛异常）+ `S.WorldMapSetup`（`WorldMapSetup` 世界开关 + `GameScene.TeleportToNPCCost`，uint 转换）。`InitInGameDialogs` 常驻创建 `BigMapDialog`（Visible=false）。`Client.Rendering/MobileAutoPath.cs` 纯逻辑控制器（仿 `MobilePickup`，seam：`Now`/`SendWalk`）——视口点击（TouchInputAdapter 同链路 `OnMouseClick`）已设 `MapControl.CurrentPath`+`AutoPath=true`（旧客户端 GameScene.Process 驱动）→ 每帧按路径终点重寻路（移动后节点陈旧）→ 跳玩家已站节点 → 节流 500ms 逐格 `C.Walk`；路径空/重寻路失败/`Cancel` → `AutoPath=false`。`MobileBootstrap` 接线：`_map` 按钮栈（紫 tint，任务按钮下方）；`ToggleBigMap`=打开→`TargetMyLocation`→`SetTargetMap(当前图)`（无记录发 `C.RequestMapInfo`）+ 关背包/装备/摇杆/HUD/拾取；`BackHandler` 地图优先关闭 + `Cancel`；`uiOpen`/`PollJoystick bagOpen` 追加 `BigMapDialog.Visible`；**寻路上升沿检测**（`_prevAutoPath`）：视口点击设 AutoPath 瞬间自动关地图窗（行走在世界中，且 uiOpen 守卫会暂停寻路 tick）；`RenderHud` 绘制地图 + `UiText.WarmTree`。**对照决策**：传送确认（旧客户端 `MirMessageBox` YesNo，未移植）→ 直接 `C.TeleportToNPC`（`TeleportToNPC` Gold 门控 `GameScene.Gold < TeleportToNPCCost` 不发包，CanTeleportTo 门控）；`ClientMovementInfo/NPCInfo.Location` 为 Shared `System.Drawing.Point` → 转换 `MPoint`；NPC 传送行/世界地图按钮随 PC 语义。验证：`Build/mapverify.ps1` → `[mapverify] PASS cases=8`（NewMapInfo 记录+按钮/行父级契约/WorldMapSetup 世界按钮+花费+世界地图打开/Show 无记录发请求→回填→重开直显/视口点击寻路设定+路径非空/移动按钮→目的地+发请求/NPC 行选中切换+CanTeleportTo 传送门控/传送 Gold 门控/`MobileAutoPath` 逐格走位+取消）+ 回归 gamesessionverify PASS + questverify 9/9。**探针踩坑**：`MirControl.Visible` getter 依赖 `Parent.Visible` → 断言对话框内控件可见性须先 `Show()`；`Enabled` 为 `internal get`（Editor 程序集不可读）→ 传送门控改行为断言（Gold/CanTeleportTo 发包与否）；裸 `Functions` 在 `Crystal.Client.Rendering` 解析到全局 Shared（`System.Drawing.Point` 签名）→ 须 `global::Client.MirObjects.Functions`（对齐 MobilePickup/MobileCombat）；`MirButton`/`SoundList` 在 `Client.MirControls`/`Client.MirSounds` 命名空间 → GameSession 补 using。**下一步**：8-4-3 小地图触控化。

- **8-3-3 完成（2026-08-08）**：仓库存取触控收口。服务端语义钉死：`C.StoreItem{From=背包格,To=仓库格}`=单向存（目标仓库格须空）、`C.TakeBackItem{From=仓库格,To=背包格}`=单向取（目标背包格须空），回声 `S.StoreItem`/`S.TakeBackItem` 成功才本地换（无 swap/占位替代）。打开流程：NPC `[@STORAGE]` → `S.UserStorage`（填 `GameScene.Storage` 80 格）+ `S.NPCStorage`（关对话）。`GameSession` 新增 4 派发：`UserStorage`（填格+`StorageDialog.Show` 连带开背包）/`NpcStorage`（关 NPC 弹框）/`StoreItem`+`TakeBackItem`（`ApplyStorageSwap`：Success→本地交换+`RefreshStats`，失败仅解锁）；`InitInGameDialogs` 常驻创建 `StorageDialog`。存取交互放 Core `StorageDialog.OnGridClick`（对齐 `NPCGoodsDialog.BuyItem` 模式）：存=选中背包格（`GameScene.SelectedCell` 为 Inventory 且有物品）→ 点空仓库格（目标被占静默，服务端权威）→ `C.StoreItem`；取=点有物品仓库格 → 扫背包首空格（`BeltIdx` 起）→ `C.TakeBackItem`；双格 `Locked` 防重复双击，回声解锁（成功失败都解锁）。`MobileBootstrap` 守卫：`BackHandler` 仓库关闭（NPC 之后、商店之前）；`uiOpen`/`bagOpen` 追加 `StorageDialog.Visible`。**对照决策**：`MirItemCell.OnMouseClick` 会先把有物品格自身设为 `SelectedCell` 再触发 Click → 若 Click 时读选中态，"选背包格→点已占用仓库格"误判成取出 → 用 `MouseDown` 事件快照按下前选中态（`_downSelection`），占用格静默；`InventoryDialog.Grid` 下标≠物品槽位（`Grid[0].ItemSlot=6`）→ 回声解锁须按 `ItemSlot` 扫描定位真实格，仓库 `Grid[to]` 下标即槽位（存=To、取=From 双向映射）。仓库密码全套/RentButton 租赁/升级仓库 `GetCell` 不移植（依赖对话框/未用，裁剪注释已记录）。验证：`Build/storageverify.ps1` → `[storageverify] PASS cases=9`（UserStorage 填格+弹框+背包连带/NPCStorage 关NPC+弹框/选背包格点空仓库格存+双格锁/目标被占静默不发包/点有物品格取+找首空格/StoreItem 回声交换+解锁/TakeBackItem 回声/分页切换可见性+按钮态/关闭隐藏）+ 回归 shopverify 9/9 + npcverify 6/6 + baginteract 7/7 + equip 5/5 + useitem 6/6 + pickup 6/6。**探针踩坑**：`StorageDialog.Show()` 在 Size=0×0 时把背包摆到 (5,0)，撑大仓库面板后两者重叠截获背包点击 → 探针 prep 后把背包右移 `Size.Width+5`；仓库格 MouseDown 快照依赖 MirControl `MouseDown` 事件先于 OnMouseClick 触发。**下一步**：8-4-1 任务四窗。

- **8-3-2 完成（2026-08-08）**：商店买卖触控收口。`GameSession` 补 `S.NewItemInfo` 派发（此前运行时 `ItemInfoList` 恒空）+ `S.NPCGoods` 派发 → `NpcGoods` handler——逐商品 `GetItemInfo`（`ItemInfoList` 查 Index）解析 `Info`，未收录跳过（防 `AddGoods` 对 null-Info 的 `DisplayGoods.Any(x=>x.Info.Index…)` NRE）；设 `GameScene.NPCRate`；`NPCGoodsDialog.NewGoods` + `Show()`（连带开背包）；`InitInGameDialogs` 常驻创建 `NPCGoodsDialog(PanelType.Buy)` 默认隐藏（同 NPCDialog 模式，运行时商店均 Buy 面板，懒建兜底才用 p.Type）。`MobileBootstrap` 移动守卫：`BackHandler` 商店关闭插入在 NPC 对话之前（顶层先关）；`uiOpen`/`PollJoystick bagOpen` 追加 `NPCGoodsDialog.Visible`（面板开时暂停摇杆/战斗/拾取）。购买链路复用 `NPCGoodsDialog`（PC 迭代包3）：点格选中→`BuyButton`→`C.BuyItem{ItemIndex=UniqueID, Count=maxQuantity, Type}`，`Count` 按 StackSize/金币/listing Count 三封顶对齐旧客户端；`CloseButton` 关闭。**对照决策**：只做**购买侧**（`NPCDropDialog` 出售未移植，已知限制）；数量沿用最大可购（`MirAmountBox` WinForms 键盘 UX 不接入，SoftKeyboard 接线推迟）。验证：`Build/shopverify.ps1` → `[shopverify] PASS cases=9`（NPCGoods 分发+Info 解析+NPCRate+背包连带/未收录商品跳过不崩/点格选中+切换/单件 Count=1/叠放整组 Count=listing/金币封顶 3000/1000=3/StackSize 封顶/未选不发包/关闭隐藏）+ 回归 npcverify 6/6 + baginteract 7/7 + equip 5/5。**探针踩坑**：`UserItem.Price()` 返回**单价×Count**（ItemData.cs:541）→ `Price()/Count` 才是单价，金币封顶用例须 Gold=单价×N；空库下面板 AutoSize 回退 0×0 → 显式关 AutoSize 设尺寸（同 NpcVerify）；跨 case 静态 `_lastClickTime` 使首击误判双击 → `CMain.Time` 全程单调递增。

- **8-3-1 完成（2026-08-08）**：NPC 对话触控收口。`Client.Rendering/MobileNpc.cs` 纯逻辑控制器（仿 `MobilePickup`，seam：`SendCallNpc`）——地图 tap（ui 空间）→ 屏转格（复用 `MobilePickup.ScreenToTile`）→ 最近 `NPCObject`（≤TapRadius=1）命中 → 置 `GameScene.NPCID`（运行时选项点击依赖它）+ 发 `C.CallNPC{ObjectID, Key="[@Main]"}` 拉对话首页；无 NPC/超半径拒绝（落回拾取）；**独立节流** CallCooldownMs=5000（`long.MinValue` 哨兵短路防减法溢出，期内消费不重发）。**接线**：`MobileBootstrap` 摇杆 tap 回调 `_npc.TapAt` 优先、`_pickup.TapAt` 兜底；`GameSession` 新增 `S.NPCResponse` 分发 → `NpcResponse` handler → `NPCDialog.NewText` 渲染选项 + Show；选项点击走 TouchInputAdapter → GameScene.OnMouseClick → `NPCDialog.ButtonClicked` → `C.CallNPC[动作]`（`Network.Enqueue` 直发，节流走 `GameScene.NPCTime`+5000）；`@Exit` 关闭（不节流）。对话框 InitInGameDialogs 预建（Visible=false，防 MapObject NPC 移除 NRE）。**对照决策**：点击节流用**独立** 5000ms 不共享 `GameScene.NPCTime`——旧客户端共享计时会在开框后吞掉首个选项点击（quirk），触控版让选项即点即响。验证：`Build/npcverify.ps1` → `[npcverify] PASS cases=6`（命中发包+NPCID/无 NPC 拒绝/超半径拒绝/独立节流 5000ms 消费不重发冷却后重发/对话框已开不重弹/NPCResponse 渲染+选项点击发包+选项节流+@Exit 关闭）+ 回归 pickup 6/6 + baginteract 7/7 + equip 5/5 + useitem 6/6 + mobileui 7/7 + mobilehud 7/7 + PC 回归 `net-npc.ps1`（NPC 对话链路）+ gamesessionverify PASS。**下一步**：8-3-2 商店买卖。

- **8-2-5 完成（2026-08-08）**：地面拾取触控收口。`Client.Rendering/MobilePickup.cs` 纯逻辑控制器（仿 `MobileCombat`，seam：`Now`/`SendWalk`/`SendPickUp`）——地图 tap（ui 空间）→ 屏转格（`ItemObject.Process` 世界→屏幕逆变换：`tileX = ui.X/CellWidth - OffSetX + user.Movement.X`）→ 最近 `ItemObject`（tap 距 ≤TapRadius=1 且距玩家 ≤PickupRadius=3，防误触远走）设目标；目标格==玩家格 → `C.PickUp`（节流 PickupCooldownMs=200，对齐旧客户端 `PickUpTime+200` 同源）；否则 `PathFinder` 逐格 `C.Walk`（物品格非 Blocking 直达，走格节流 500ms 与摇杆/战斗同频）到格即拾取；目标被拾取移除（`S.ObjectRemove` → `MapObject.Remove`）→ 自动清目标。**服务端语义钉死**：`PlayerObject.PickUp()` 仅拾取玩家**当前所在格**一次一物 → 逐格走位到格是必须（非优化）；`C.PickUp` 空包，确认走 `S.ObjectRemove`。接线：`MobileBootstrap` 摇杆 Up 且无拖拽位移（`!ReleasedWithIntent`）且非 HUD 按钮区（`MobileHud` 新增公开 `Hit(ui)` 谓词）→ tap；摇杆 Down/拖拽/背包·装备面板打开 → `Cancel`（移动优先）；拾取目标激活时 `Update` 让位 `_pickup.Tick()`（战斗索敌不覆盖走位）。**对照决策**：拾取反馈（选中框/飘字）不移植——物品移除即反馈（KISS/YAGNI）。**已知限制**：`FloorItems` 图集不在仓库 → 运行时地面物品未生成（`GameSession.ObjectItem` 早退），控制器作用于对象模型，待数据落地即用。验证：`Build/pickupverify.ps1` → `[pickupverify] PASS cases=6`（命中+邻格 tap/无物品/距离外拒绝/相邻拾取+节流+冷却重发/目标移除清目标/两格寻路走位到格后拾取）+ 回归 useitem 6/6 + equip 5/5 + baginteract 7/7 + mobileui 7/7 + PC 回归 `net-interact.ps1`（PickUp 路径）。**下一步**：8-3-1 NPC 对话。
- **8-2-4 完成（2026-08-08）**：药品使用触控收口。`MirItemCell.UseItem` 新增药水/卷轴/书/食物分支 → `C.UseItem`（锁格防重复双击）；`GameSession` 新增 `S.UseItem` 分发 + 回流 handler（internal + `InternalsVisibleTo` 供探针）——成功→`cell.Item.Count--`（末瓶清格）+ `RefreshStats`，失败→仅解锁不扣数。**满血不溢出**：HP/MP 恢复走独立 `S.HealthChanged`（服务器权威封顶），客户端不本地补血 → 溢出天然不可能（回流后 HP 恒不变，探针钉死）。**对照决策**：旧客户端药水 Shape==4 确认框（MirMessageBox）与腰带自动移动（C.MoveItem）不移植——无移动 MirMessageBox，双击即触控确认（KISS/YAGNI）。验证：`Build/useitemverify.ps1` → `UseItemVerify` 探针 6/6 PASS（双击发包+锁格/数量-1/不本地补血 HP 不变/失败回流不扣数/末瓶清格/非药水走装备链）+ 回归 baginteract 7/7 + equip 5/5 + mobileui 7/7 + softkeyboard 8/8 + resolution 14/14 + bag 8/8。**下一步**：8-2-5 拾取。
- **收口（2026-08-08）**：sanduan 提取工作流完成——P0/P1/P1/P2/P2 五项全 ✅（对象模型 24/24、描边 3/3、光源脉冲 3 时刻、软键盘 8/8、分辨率 14/14），`docs/sanduan-extraction.md` 顶部补收口摘要（含运行时接线遗留：`GameScene.DrawLights` 光源渲染待阶段8 UI 接线、`SoftKeyboardBridge.Focus(box)` 待移动登录/聊天 UI）；P3 MirMath seam 对照为**随用随查**参考项非排期。`docs/three-platform-migration-plan.md` 当前在途改回 8-2-4。**恢复开发文档剩余任务**：8-2-4 药品使用 → 8-2-5 拾取 → 8-3-1 NPC 对话 → …
- **P2 完成（2026-08-08）**：sanduan 提取 C2 分辨率缩放统一（`docs/sanduan-extraction.md` 优先级表第 5 项）。`Client.Rendering/ScreenMetrics.cs` 移动 UI 屏幕尺寸**单一扇出点**——渲染真值（`GameRuntime.ScreenW/H`）→ 触摸 y 翻转基准（`MobileUiAdapter.ScreenW/H`）+ 对话框布局（`Settings.ScreenWidth/Height`）；`MobileBootstrap` Start/Update 与 `GameSession.InitInGameDialogs` 三处手动分散同步统一路由 `ScreenMetrics.Set`（消灭 X-1 y 镜像同类根因：翻转基准与渲染高度漂移）。**对照决策**：sanduan `SizeRatio` 黑边缩放是**死代码**（恒 1.0 从未赋值、`OnRenderObject` 缩放矩阵整段注释、`ScreenToWorld` 退化 1:1），且与本项目原生 1:1 渲染 + 边缘锚点自适应布局（HUD/背包随屏重算 + MinTouchSize 44px 命中下限）语义重复 → **不引入黑边缩放**，坐标契约（纯镜像 y 翻转/无缩放无黑边/边缘锚点重算/触控下限）由探针钉死。验证：`ResolutionVerify` 14/14 PASS `[resolutionverify] PASS cases=14`（扇出 + 早退 no-op + ToUi 屏角精确镜像/中点不动/换分辨率基准更新 + HUD 720p/1080p 锚点 + 背包 720p/1080p 锚点 + SetMargin 重算 + MinTouchSize 下限 + 合规尺寸不缩）+ 回归 mobileuiverify 7/7 + softkeyboardverify 8/8。**下一步**：提取收口（优先级表全 ✅）+ 恢复开发文档剩余任务。
- **P2 完成（2026-08-08）**：sanduan 提取 C1 Android 软键盘桥（`docs/sanduan-extraction.md` 优先级表第 4 项）。`Client.Rendering/SoftKeyboardBridge.cs` 纯逻辑驱动核心 + `ISoftKeyboard` seam（与 Unity TouchScreenKeyboard 解耦，可确定性探针）；sanduan 的 `GUILayout.TextField + FocusControl` 聚焦路由（IMGUI 为阶段5 ADR 所禁）换成 `MirTextBox.InputTextBox` 纯 C# 输入模型 + 控件树 `KeyPress` 路由——`Focus(box)` 开软键盘（初始文本/密码/最大长度走框属性）、`Poll()` 每帧同步文本 + Enter 提交（`KeyPress(Enter)` 进控件树→ChatDialog/登录同链触发）+ 取消/解绑关键盘；`UnitySoftKeyboard` 为 TouchScreenKeyboard 包装（secure 掩码走原生参数）。`MobileBootstrap` 初始化默认提供者 + Update 每帧 Poll。验证：`SoftKeyboardVerify` 探针 8/8 PASS `[softkeyboardverify] PASS cases=8`（绑定开键盘透传文本/密码/最大长度、轮询同步、Enter 提交→KeyPress(Enter)+解绑、取消解绑、显式解绑、重复绑定先解旧、null 安全）。**注**：移动登录/聊天 UI 尚未建成（mobile 登录用 `MobileConfig` 硬编码凭据），桥为能力 + 探针验证，UI 聚焦时调 `Focus(box)` 接入。**下一步**：P2 分辨率缩放统一（TouchInputMapper 对照 SizeRatio）。
- **P1 完成（2026-08-08）**：sanduan 提取 B2 光源脉冲补 R5 + AmbientLightBlend 对照（`docs/sanduan-extraction.md` 优先级表第 3 项）。`Client.Rendering/LightPulse.cs` 单一事实来源（`Brightness=0.975+0.025·sin(9t)`、`Alpha=0.975+0.05·sin(9t)`，峰值 1.025 GPU 输出饱和 → clamp 保证字节确定性）；`LightRender` 增 `CRYSTAL_TIME=<秒>` 脉冲模式——每灯 tint 经 `LightPulse.Modulate` 调制（GPU 绘制色与 CPU 期望同走公式），未设保持 R5 静态行为。验证：`Build/lightpulseverify.ps1` 三确定性时刻全过 `[lightpulseverify] PASS`（t=0 基线 b=a=0.975 / t=π/18 峰值 b=1,a=1 / t=π/6 谷底 b=0.95,a=0.925）+ R5 静态回归三检 fail=0。**AmbientLightBlend 对照决策**：静态光照贴图覆盖（overlay.a>0 整像素替换）无每灯 tint/无脉冲，R5 动态多通道语义超集 → 维持 R5，不引入。**注**：运行时 `GameScene.DrawLights` 尚未接线（R5 探针级能力），LightPulse 供接线时调用。**下一步**：P2 Android 软键盘桥（MirTextBox + TouchScreenKeyboard）。
- **P1 完成（2026-08-08）**：sanduan 提取 B1 OutLine 描边 shader 复刻（`docs/sanduan-extraction.md` 优先级表第 2 项）。`Shaders/CrystalSpriteOutline.shader` 输出**平涂描边色**（保留 sanduan 阴影例外：a<0.5 / (16,8,8) / r<0.01 丢弃，`Blend SrcAlpha OneMinusSrcAlpha`）；`CrystalSpriteBatch` 新增 `SetOutline/SetOutlineColour/DrawOutline(tex,src,pos,colour,outlineColour,thickness)`——4 向 ±thickness px 偏移描边色副本 + 原图压顶 = 轮廓外 1px 描边光环（MirLabel 文本描边同款，**图集兼容**；sanduan 的 frag UV 邻域采样会串读相邻帧，不可用）。验证：`OutlineVerify` 探针 3 用例逐像素对照（t=1/t=2/alpha0.5；白方块光环 + 阴影/近黑例外不描边 + 原图压顶原样，±2 容差）全过 `[outlineverify] PASS cases=3` + `Build/outlineverify.ps1`。**踩坑**：shader 首版编译错误 `texcol.rgb == fixed3(...)` 向量比较 d3d11 需 `all()` 包裹 → 洋红错误材质（实证 sanduan shader 不可照搬，须经本项目编译+逐像素验证）。
- **P0 完成（2026-08-08）**：sanduan 提取 A1/A2 对象模型补全（`docs/sanduan-extraction.md` 优先级表第 1 项）。`Ported/SpellObject.cs` 逐字移植（33 个 Spell case 帧选择：Magic/Magic2/Magic3/Effect/Dragon/Monsters 库 + DrawFrame/FrameInterval/FrameCount/Repeat/Blend/Light + 地图特效/音效副作用；Process 帧推进 Repeat 回绕 Ended/单次不绕；DrawBlend 混合 0.8F；Monster BOSS 法术 Process 内声音/末帧定格特例）+ `Ported/ItemObject.cs` 逐字移植（ObjectItem 装载 + ObjectGold 金币分级帧 112-116 `<100/<200/<500/<1000/else` + Process 世界→屏幕居中换算 + FinalDrawLocation + DisplayRectangle + DrawName 双重载）。`Libraries` 新增 `FloorItems` 槽位（图集数据不在仓库 → `EnsureMLibrary` null → 不建对象优雅跳过，数据就位补图集即可）；`GameSession` 新增 `ObjectSpell`/`ObjectItem`/`ObjectGold` 派发（internal + `InternalsVisibleTo` 供探针）。验证：CoreVerify 0 错误 + `ObjectModelVerify` 探针 24/24 PASS（SpellObject 帧选择/帧推进回绕/非回绕/特效副作用/DrawLocation 换算、GameSession 派发落对象+无图集跳过、ItemObject 装载/居中/金币分级帧）+ `Build/objectmodelverify.ps1` runner。
- **8-2-3 完成（2026-08-08）**：装备穿戴触控收口。双击背包格→`C.EquipItem`（`MirItemCell.OnMouseDoubleClick`→`UseItem`，`CanWearItem` 性别/职业/等级/负重资格校验，可穿戴锁来源+目标双格防重复双击，等级不足静默拒绝不发包；Bracelet/Ring 右手优先让位逻辑对齐旧客户端）；双击装备格→`C.RemoveItem`（`RemoveItem` 从 `BeltIdx` 起扫首个空背包格）。回流：`GameSession` 接 `S.EquipItem`/`S.RemoveItem`（internal + `InternalsVisibleTo` 供探针）→ `UserObject.ApplyEquip/ApplyRemove` 按 UniqueID 数组交换 + `RefreshStats` 属性重算 + 双格解锁；`EnsureUser` 建 `CharacterDialog(MirGridType.Equipment, user){ Parent=Scene, Visible=false }`（**修复真实 bug**：sanduan 参照 `Parent=this`——Mir 鼠标链 hit 走 `Scene.Controls` 子树递归，漏挂父则装备格收不到悬停/双击，卸下永不触发）。`MobileBootstrap` 右上装备按钮（`MobileBag` 复用 `SetMargin` 下移 + 绿色 tint）+ `ToggleChar`（开=ShowCharacterPage+与背包互斥+Cancel 摇杆/HUD+清选中，关=Hide）接入。`EquipVerify` 探针 5/5 PASS（可装备发包+锁双格/等级不足拒绝/`S.EquipItem` 回流数组+属性+解锁/卸下发包/`S.RemoveItem` 回流）+ 回归 `baginteractverify` 7/7 + `bagverify` 8/8 + CoreVerify 0 错误。**探针踩坑**：双击窗口 `CMain.Time` 每次重置 1000 会时间倒退命中上轮残留 `_lastClickTime` 把首击误判双击、窗口清空后次击落单击 → 单调递增时钟；batchmode Prguse 库 null → `CharacterPage` Size 退化 (0,0) → `IsMouseOver` 假使装备格子树不可达（鼠标链停在 Dialog 本级）→ 显式关 AutoSize 设 240x260；`ItemInfo` RequiredGender/RequiredClass 默认 0 → `HasFlag` 必失败，探针物品需显式 Male/Warrior。**下一步**：8-2-4 药品使用。
- **8-2-2 完成（2026-08-08）**：背包面板触控交互收口。`MirItemCell.OnMouseClick` 点格选中（有物品→`SelectedCell` 边框高亮+图标置灰；空格→清本网格选中，跨网格守卫）+ `InventoryDialog.ClearSelection` 选中生命周期（Reset 切页/Hide 关闭清选中+Tooltip+HoverItem）+ `MobileBootstrap` 返回键/关背包走 `Hide()`。`BagInteractVerify` 探针 7/7 PASS（点格选中+Tooltip/空格取消/越界忽略/切页状态/切页清选中/关闭清选中/任务页，exit 0）+ 回归 `bagverify` 8/8 PASS + CoreVerify 0 错误 + PC 回归 `net-bag` PASS（bag ok exit 0）。**探针踩坑**：batchmode 无库数据下 `MirImageControl.Size` getter（AutoSize&&Library!=null&&Index>=0 → `GetTrueSize(Index)=(0,0)`）吞显式尺寸 → 面板/按钮 `Size=0` → `IsMouseOver` 永不命中，探针统一关 AutoSize 回落 base.Size（面板 340x240 覆盖 CloseButton 全宽 289..329）；`_user.Inventory = new UserItem[56]` 数组替换会丢原物品，case5/6 重选前需保格。
- **8-2-1 完成（2026-08-08）**：移动背包按钮 + InventoryDialog 触控接入收口，打 tag `stage8-bag-v1`。`MobileBagVerify` 探针 8/8 PASS（命中 toggle/按钮外不触发/消费语义/Cancel 不 toggle/连点翻转/松手容错/屏幕重设重布局/开态跨重设保留）+ `mobilehudverify` 7/7 + CoreVerify 0 错误 + PC 回归 `net-bag` PASS（exit 0 bag ok）+ 模拟器冒烟 `androidverify -Smoke` PASS（shot1 含背包按钮：物理 (2096,210)-(2230,290) 中心 (2163,250) 精确匹配，10901 像素填充 99.7%）。修复真实 bug：Cancel 后残留 Up 走松手容错误 toggle → `MobileBag`/`MobileHud` 加 `_canceled` 抑制位。
...
**阶段7 第 4 项（移动资源包、下载与版本校验，2026-08-06 完成）**：①**清单工具**：`AssetCompiler` 新子命令 `manifest <dir> --out <file.json>`（`tools/AssetCompiler/Manifest.cs`，net8.0）——递归扫目录每文件输出 `{rel,size,sha256}` + 汇总 `count/totalBytes`，确定性排序（OrdinalIgnoreCase）跳过输出文件自身；AArmour 库实测 96 文件 121MB 清单正确；②**`ResourceSync`**（`Client.Rendering` runtime 静态工具类）：`BuildLocalIndex`（本地目录→rel→size/sha256 索引）→ `PlanDiff`（远端清单 vs 本地→需下载列表：缺失或 size/sha256 任一不一致，按清单顺序）→ `DownloadFile`（`UnityWebRequest` GET→落盘→sha256 校验，不匹配删脏文件不留残留）；本地布局约定镜像 assetcompile（manifest 与页 PNG 同目录，兼容 `AtlasLibrary.Load` 即下载后可加载），全量资源打包/增量清单属阶段8；③**`ResourceSyncVerify` 探针**（Editor batchmode）：场景1 diff（变更 b.bin + 缺失 c.bin 检出、相同 a.bin 排除）+ 场景2 端到端（`TcpListener` 起本地静态 HTTP 资源服务器→正确 hash 下载落盘一致、错误 hash 拒绝且 dest 无残留）→ **`[resourcesync] PASS cases=2` exit 0**（日志 `Unity/Build/resourcesync-verify.log`）。Unity 侧 hash 用 `SHA256.Create()`+自写 hex（`Convert.ToHexString` 为 .NET 5+ API，Unity .NET Standard 2.1 不可用）。

**阶段7 第 5 项（设备性能分级，2026-08-06 完成）**：`DeviceCapability.cs`（`Client.Rendering` runtime）：①**分级决策表** `Classify(DeviceProfile)`——纯函数（探针注入 profile 断言，无 Unity 依赖）：内存≥8GB 且核数≥8 且 GPU 上限高（`MaxTextureSize≥8192` 且 GPU 显存≥4GB）→ High；内存<4GB 或核数<6 或 GPU 明显低（`MaxTextureSize<4096` 或显存<1GB）→ Low；其余 Medium（对齐 PRD 11.2 L/M/H 三级）；②**真实采样** `SampleUnity()`（`SystemInfo.processorCount/systemMemorySize/graphicsMemorySize/maxTextureSize`）；③**档位配置** `TierQuality`——五维度对齐 PRD "内部渲染分辨率/粒子密度/阴影灯光/远处对象更新/纹理等级"（`RenderScale 1.0/0.85/0.7`、`ParticleScale 1.0/0.75/0.5`、`ShadowQuality 2/1/0`、`DrawDistanceScale 1.0/0.8/0.6`、`TextureLevel 0/1/2`），消费点（动态降级应用）属阶段8；④**`DeviceTierVerify` 探针**（Editor batchmode）：5 组注入 profile 分级断言 + 三档配置单调性 → **`[device-tier] PASS cases=2` exit 0**（日志 `Unity/Build/devicetier-verify.log`）。注：`-nographics` batchmode 下 `SystemInfo.graphicsMemorySize` 失真（headless 128MB）致真实采样归 Low，仅采样局限；真实档位以实际设备 SystemInfo 为准。

**阶段7 第 6 项（iOS Host 与签名配置骨架，2026-08-06 完成）**：`BuildIOS.cs`（`Client.Rendering.Editor`）：①**iOS PlayerSettings 配置** `Configure()`——`SetApplicationIdentifier(iOS, com.crystal.mir2)`（与 Android 同 bundle）、`targetOSVersionString=13.0`（持久化 `iOSTargetOSVersionString`，横屏 LandscapeLeft 已在全局持久化共享）、**签名入口** `CRYSTAL_IOS_TEAMID` env → `appleDeveloperTeamID`（未设留自动签名占位）；②**场景幂等挂载**（`EnsureComponent`，AppLifecycle/TouchInput 平台无关，与 Android Host 共享 Main 场景）；③**`BuildIOSVerify` 探针**（Editor batchmode）：Configure 后断言 bundle/横屏/minOS 非空 + 签名入口状态 → **`[build-ios] PASS cases=3` exit 0**（日志 `Unity/Build/buildios-verify.log`）。**Windows 边界**：Xcode 工程构建 + 真机签名 + TestFlight 需 macOS（PRD 行 883 风险项）——Windows 只完成"配置流水线"这半并验证，完整双端 G7 门禁在 macOS 环境补齐（backlog 登记）。

**阶段6 C5（PC Player 构建 + 真实屏幕渲染验证，2026-08-07 完成）**：`BuildPC.cs`（`Client.Rendering.Editor`）：①**PlayerSettings**（company=Mir2/product=Crystal/1280×720 windowed/runInBackground）；②**场景接线**（幂等：Main 主相机 SolidColor 清屏触发 OnPostRender + GameBootstrap + AppLifecycle）；③**shader 打入构建**——`GameBootstrap.renderShaders` 序列化 4 个 Crystal/Sprite*.shader 引用（须 SaveScene 前赋值，否则构建剥离 → 运行时 Shader.Find null）；④`BuildPipeline.BuildPlayer` StandaloneWindows64。**验证 `Build/pcverify.ps1`**（起 Server.exe→等 7000 端口→起 Crystal.exe（env 注入 map/atlas 目录 + CRYSTAL_AUTO_SHOT）→等 `[pcplayer] shot` + PNG>10KB 断言）：**PASS——boot→login→选号→enter→渲染→自动截图，`objects=21 dps=26`，产物 `Build/PC/Crystal.exe` 90MB + `Build/pcplayer-shot.png` 1920×1080（像素分析 minLum=2 maxLum=249 avgLum=119.1 uniqueColors=776——真实游戏画面，非纯色）**。**三层全黑根因链（C5 关键实证）**：①`Graphics.DrawMeshNow` 在相机 OnPostRender 上下文不输出（RT 探针正常 vs PC 全黑）→ 改 `GL.Begin/End` 立即模式（顶点 CPU 烘焙 NDC + shader vert 直通）；②**`RenderTexture.active=null` 使 OnPostRender 内 GL 立即模式绘制静默丢失**（最终根因：诊断红 quad 不经 `Begin` 成功、Flush 经 `Begin` 全黑）→ 屏幕模式（target=null）不再触碰 active/GL.Viewport（RT 模式仍正常，gameruntimeverify 回归 PASS）；③`Clear` 屏幕模式靠相机清屏（SolidColor 0.1,0.1,0.1 即截图背景 25）。**其余坑**：MapLibRel smTiles 段映射偏移（idx 111-118 → `smTiles2..smTiles9` 非 `SmTiles+idx-99`）+ EnsureLib/EnsureMapLib 负缓存防日志刷屏（missing 缓存 null 防每帧 File.Exists+LogWarning，日志从 11 万行降到 44 行）；**Player.log 每次启动 truncate（非 append）**→ pcverify 行号过滤失效（`$startLine` 误过滤本次 shot 行）→ 启动前删 Player.log 替代行号过滤。**回归**：`gameruntimeverify.ps1` RT 模式 PASS（Begin 改动未破坏 batchmode 探针）。**C4 uGUI UI 待办**（PC Player 界面层，backlog 登记）。

**阶段8 前置（Android APK 登录→进图→渲染→移动，2026-08-07 完成）**：`AssetCompiler` 新子命令 `subset --map <nn0.map> --center x,y --radius N --atlas <map/> --out <dir>`（`tools/AssetCompiler/Subset.cs`，内嵌 Type100 MapReader 解析）——按地图中心区域引用帧提取重打包小图集，**输出兼容 `AtlasLibrary.Load`**（Images 数组长度=原段 Count，选中帧保留 Page/X/Y，未选中 `Empty=true,Page=-1` 不占纹理；Pages 重排仅含选中帧）。实测 nn0 中心 60 格 → 14 libs 3023 frames 88.5MB RGBA（全图 337MB → 区域裁剪可行）。**`MobileBootstrap.cs`**（`Client.Rendering` runtime，Android 引导壳）：连接 10.0.2.2 → 自动登录 → 自动选号 → 进图 → `GameRuntime.TickLogic/RenderScreen` + `PollSwipe` 滑动 8 向走格（复用 `C.Walk` 同 PC 通道）；配置 BuildAndroid env 注入（`CRYSTAL_NET_HOST/PORT/LOGIN_ID/PW`，缺省 `10.0.2.2:7000 pcplayer/pcplayer`）；平台互斥（非 Android disable 自身 + `GameBootstrap.cs` Android disable，Main 场景双组件共存单活）；日志 `[mobile] connect/login/select/enter/user@x,y/render-ready` 供验证断言。**`BuildAndroid.cs` 扩展**：`EnsureCamera`（SolidColor 清屏触发 OnPostRender）+ `EnsureMobileBootstrap` 幂等挂载 + env 注入。**验证 `Build/androidverify.ps1`**（起 Server.exe→起模拟器 Medium_Phone_API_36.1→构建 APK→subset 裁剪→adb install/push（nn0.Map + mapAtlas，目录 push 逐文件相对路径 + run-as 回退）→**两步启动**：首次 launch 解析 logcat 实际出生坐标 `[mobile] user@x,y` → 按实际坐标重裁 → 重推 → force-stop 重启 → 最终断言链 connect→login→select→enter→user@ + render-ready 等渲染 + 截图色数 + `adb input swipe` 移动断言位置变化）：**PASS exit 0——chain connect/login/select/enter/user 全 true，coords=290,616，moved=True（290,616→291,616），shot1=4453643 bytes colors=232563（真实地图渲染非纯背景）**。**四个踩坑（修复）**：①**render-ready 判据**：原用 `LastObjectDraws`（本帧绘制对象数）——出生点周围无怪/NPC（logcat `enter-game objects=0`）→ drawn=0 → 永不触发 → E2E step 7 空等 120s 超时 → 改 `GameRuntime.FramesRendered`（新增计数器，首帧渲染完成即就绪）；②**模拟器检测**：deviceModel=`sdk_gphone64_x86_64` 不匹配 `"Android SDK built for"` → Screen.SetResolution(1280,720) 降级（模拟器 SwiftShader 2400x1080 帧率<5 触摸 Began 帧合并丢失）从未生效 → 改 `SystemInfo.graphicsDeviceName.Contains("SwiftShader")`（真机 GPU 非软渲染保持原生）；③**PS 5.1 字符串比较**：python 输出的 `"232563"` 与 `50` 用 `-gt` 按字典序比较（'2'<'5'）→ False 致已满足的判定 FAIL → `[int]::TryParse` 强转；④**两步裁剪**：首次中心 350,350 裁剪覆盖出生点 → 实际坐标 290,616 重裁 60 格（14 libs）→ 重推 → 重启（玩家出生点不在中心区域的自动化对策）。**不做（YAGNI/留阶段8 主任务）**：HTTP OTA 下载（ResourceSync Android 落地）、对象库动态加载、纹理分级动态降级（DeviceCapability 消费点）、战斗触控 HUD。

**阶段8 第1项（战斗触控 HUD）增量1（触控移动摇杆，2026-08-07 完成）**：`TouchJoystick.cs`（`Client.Rendering` runtime 纯逻辑层，与 Unity Input 解耦可确定性单测）：浮动摇杆——触摸按下处即底盘原点，拖拽向量 8 向量化（`Mathf.Atan2` 标准角→顺时针从 Up 起 45°/格，对齐 `MirDirection` Up=0…UpLeft=7）+ **死区 `DeadZonePx=12px`**（防手抖）+ **奔跑阈值 `RunThresholdPx=64px`**（超阈值 `C.Run`）；**单指锁**（已激活时新 Down 忽略，防多指抢锁）、**拖回死区保持方向不抖动**、**`LastDir/LastRun` 松手复位后保留**（Adapter 快速轻滑补一步）。`MobileBootstrap.cs` 替换 `PollSwipe`→`PollJoystick`：`Input.touches` 映射 `JoystickPhase`（Began/Down、Moved/Move、Ended/Up、Canceled/Cancel、Stationary 保持）喂 `OnTouch`，移动通道与 PC WASD 同（`C.Walk`/`C.Run` 8 向，500ms 走格节流），松手若刚拖拽过补发一步。**验证 `Build/joystickverify.ps1`**（Unity batchmode 探针 `TouchJoystickVerify`，无需服务器）：11 用例断言死区边界（恰等移动/恰小不动）、奔跑阈值、8 向量化（四正四对角）、多指忽略（第二指 Down/Move 不影响主指）、松手 `LastDir/LastRun` 保留、Cancel 复位、轻点从未移动、拖回死区方向保留 → **`[joystickverify] PASS cases=11` exit 0**。**两个修复**：①`OnTouch` Down 无条件赋值 `_touchId` 致第二指抢锁（case6）→ 加 `if (_active) return;` 守卫；②测试坐标 bug（case3 位移 100px≥64 应为 50px；case5 上/下用了对角坐标）→ 修正为纯方向位移。**模拟器 E2E 回归 `androidverify.ps1`（swipe 移动断言走摇杆松手补步路径）PASS（2026-08-07）**：login→enter→render→swipe **moved=True coords=294,616→293,616**（多方向 swipe 西→东→北→南任一动即 PASS，出生点可能邻墙固定东滑会撞墙）。**⚠️ 奔跑需助跑（E2E 移动失败根因，决定性实证）**：服务器 `HumanObject.CanRun`（Server/MirObjects/HumanObject.cs:131）要求 `_stepCounter>0 || FastRun`——`_stepCounter` Walk 成功时递增（2478）、静止超 700ms 清零（`CellTime+700<Envir.Time`，259）、Run 不消费（连续 Run 间隔<700ms 步数保持）；普通玩家 `FastRun=false`。**静止直接发 `C.Run` 被拒**（发同位置 S.UserLocation，客户端 LogPosition 检测不到变化 → 移动 E2E 假 FAIL）。原 150ms 快速 swipe（位移 426px>奔跑阈值 64px）松手补步发 `LastRun?C.Run:C.Walk`=单个 `C.Run` → 被拒不动；2000ms 长 swipe 先 Walk 积累步数后 Run 有效（佐证）。**修复（MobileBootstrap.PollJoystick 助跑状态机）**：700ms 内发过移动包才切 `C.Run`，否则首包/松手补步发 `C.Walk`（补步固定一格，轻扫意图 + 静止起跑无助跑）。diag 实证全链：touch Down→Up → release-step WALK → `userloc recv 293,616` → 位置心跳更新；移除 diag 后重建回归仍 PASS。
...
| 阶段7 Android Host | `Unity/Assets/Crystal/Client.Rendering.Editor/BuildAndroid.cs` + `BuildIOS.cs` + `Assets/Scenes/Main.unity` + `Client.Rendering/AppLifecycle.cs` + `TouchInputMapper.cs` + `TouchInputAdapter.cs` + `ResourceSync.cs` + `DeviceCapability.cs` + `Client.Rendering.Editor/TouchInputVerify.cs` + `ResourceSyncVerify.cs` + `DeviceTierVerify.cs` + `BuildIOSVerify.cs` + `tools/AssetCompiler/Manifest.cs` + ProjectSettings（Android/iOS：company/product/bundle/minSdk/minOS/目标版本/图标/横屏） | ✅ APK 构建通过（`[build-android] OK`，Unity/Build/Android/crystal.apk 16MB，产物不入库）+ 模拟器实证（安装/启动/横屏 2400x1080/挂起恢复 `resume pausedMs=31029`/触控 adapter started + tap 无崩溃）+ TouchInputVerify 8 用例 PASS + ResourceSyncVerify 2 场景 PASS（diff 检出 + 本地 HTTP 端到端下载 sha256 校验）+ DeviceTierVerify 2 场景 PASS（5 组分级断言 + 档位单调性）+ BuildIOSVerify 3 断言 PASS（bundle/minOS/横屏）；G7 门禁（双端真机登录/进图/移动/重连）待 macOS 环境补齐；安全区/软键盘/iOS 构建签名推迟 | 阶段7 |
| 阶段6 C5 PC Player | `Unity/Assets/Crystal/Client.Rendering.Editor/BuildPC.cs` + `Build/buildpc.ps1` + `Build/pcverify.ps1` + `Client.Rendering/CrystalSpriteBatch.cs`（GL 立即模式）+ `GameBootstrap.cs` + `GameRenderer.cs` + `GameRuntime.cs` + `Assets/Scenes/Main.unity` + ProjectSettings | ✅ PC Player 构建 + 真实屏幕渲染验证 PASS（`[build-pc] OK` 90MB + `pcverify` PASS：boot→login→enter→render→auto-shot，截图 1920×1080 uniqueColors=776 真实画面）；**全黑根因链修复**（DrawMeshNow OnPostRender 不输出→GL 立即模式；`RenderTexture.active=null` 静默丢绘制→屏幕模式不触碰 active；smTiles 段映射偏移 + 负缓存防刷屏；Player.log truncate→启动前删 log）；RT 回归 gameruntimeverify PASS；C4 uGUI UI 待办 | 阶段6 C5 |
| 阶段8 前置 Android 引导 | `tools/AssetCompiler/Subset.cs`（区域帧裁剪子命令）+ `Client.Rendering/MobileBootstrap.cs`（Android 引导）+ `MobileConfig.cs` + `BuildAndroid.cs`（EnsureCamera/EnsureMobileBootstrap/env 注入）+ `GameBootstrap.cs`（平台互斥）+ `Build/androidverify.ps1` | ✅ 端到端 PASS（androidverify exit 0：chain connect/login/select/enter/user 全 true + moved=True 290→291,616 + shot1 4.45MB colors=232563 真实渲染）；subset 输出兼容 AtlasLibrary.Load（Images 定长 + Empty 占位）；两步裁剪按实际出生坐标；修复 render-ready 判据（FramesRendered）/SwiftShader 检测/PS 字符串比较 | 阶段8 前置 |
...
按 PRD 阶段6 第 2 项：把 batchmode 探针逻辑升级为可运行 StandaloneWindows Player。拆 C1-C5，本段覆盖 C1-C3（渲染核心/网络会话/主循环+输入壳）+ C5（BuildPC 构建接线，2026-08-07 完成）。C4 uGUI UI 待办（backlog）。
...
**C5 BuildPC（`Client.Rendering.Editor/BuildPC.cs` + `Build/buildpc.ps1` + `Build/pcverify.ps1`，2026-08-07 完成）**：①**PlayerSettings**（company=Mir2/product=Crystal/1280×720 windowed/runInBackground）；②**场景幂等接线**——Main 主相机 SolidColor 清屏（背景色即 PC 屏幕模式背景，触发 OnPostRender）+ 挂 GameBootstrap + AppLifecycle；③**shader 打入构建**——`GameBootstrap.renderShaders` 序列化 4 个 Crystal/Sprite*.shader 引用（ShaderPaths，SaveScene 前赋值 → 场景→shader 依赖 → 构建包含，Player 默认剥离未引用 shader）；④**`BuildPipeline.BuildPlayer` StandaloneWindows64**。验证编排 `Build/pcverify.ps1`：起 Server.exe→等 7000 端口→删旧截图/Player.log→起 Crystal.exe（`CRYSTAL_MAP_DIR`/`CRYSTAL_MAP_ATLAS_DIR`/`CRYSTAL_ATLAS_DIR`/`CRYSTAL_AUTO_SHOT` env 注入）→等 `[pcplayer] shot` 日志 + PNG>10KB 断言 → 清进程。**PASS：boot→login→建号/选号→enter（objects=21）→渲染→自动截图，产物 `Build/PC/Crystal.exe` 90MB + `Build/pcplayer-shot.png`（1920×1080，像素 minLum=2 maxLum=249 avgLum=119.1 uniqueColors=776——真实画面含 25 背景 + 208,188,144 等地图/对象色）**。**三层全黑根因链（实证隔离法：RT 探针正常→PC 全黑→red quad 诊断）**：①`Graphics.DrawMeshNow` 在相机 OnPostRender 上下文不输出（RT 无相机时正常）→ Flush 改 `GL.Begin/End` 立即模式（顶点 CPU 烘焙 NDC + Crystal/Sprite shader vert 直通 v.vertex.xy，GL 矩阵栈 identity）；②**`RenderTexture.active=null` 使 OnPostRender 内 GL 立即模式绘制静默丢失**（最终根因：诊断红 quad 不经 `Begin` 成功、Flush 经 `Begin`（置 active=null）全黑）→ `CrystalSpriteBatch.Begin/SetSurface` 屏幕模式（target=null）不再触碰 active/GL.Viewport，仅 RT 模式设置；③屏幕模式 `Clear` 依赖相机清屏（`RenderTexture.active!=null` 才 GL.Clear），截图背景 25 来自 SolidColor。**C5 其余坑**：MapLibRel smTiles 段映射偏移（`idx 111-118 → "ShandaMir2/smTiles"+(idx-109)` = smTiles2..9，非 `SmTiles+idx-99`，C3 视野未触发、PC 进图暴露）+ EnsureLib/EnsureMapLib 负缓存（missing 缓存 null 防每帧 File.Exists+LogWarning，日志 11 万行→44 行）；**Player.log 每次启动 truncate（非 append）**→ pcverify 原 `$startLine` 行号过滤误过滤本次 shot 行（Unity 启动重写 log，本次行号 < 启动前行数）→ 改为启动前删 Player.log（每条匹配必然属本次运行）。**回归**：`gameruntimeverify.ps1`（RT 模式，Begin active 逻辑改动后）**PASS**——RT/屏幕双路径均验证，`CrystalSpriteBatch.Begin` 兼容两模式。
