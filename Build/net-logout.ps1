# P4-M5 orchestration: start server (Server.exe standalone) -> Unity logout probe -> assert -> stop server.
# Logout probe: login -> StartGame -> C.LogOut -> assert S.LogOutSuccess{Characters} -> re-enter (C.StartGame)
#   -> assert S.UserInformation again (character state persisted across logout).
param(
...
& $unity -batchmode -projectPath $proj -executeMethod Crystal.Rendering.Editor.NetProbe.RunLogout -logFile $log | Out-Null
$code = $LASTEXITCODE
...
$ok = ($code -eq 0) -and ($line -match "logout ok")
