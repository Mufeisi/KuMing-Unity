...
{
    // 逐字移植（2026-08-06）：Client/MirScenes/Dialogs/HeroDialogs.cs
    // 英雄系统 8 控件：英雄背包（HeroInventoryDialog）/英雄腰带（HeroBeltDialog）/英雄菜单
    // （HeroMenuPanel）/英雄状态（HeroInfoPanel）/自动喝药预览（HeroAutoPotPreview）/行为
    // （HeroBehaviourPanel）/英雄管理（HeroManageDialog + HeroManageAvatar）。
    // 裁剪：CMain.InputKeys.GetKey(KeybindOptions.X)（Unity 无 KeybindOptions 配置，按键名空，
    // 与 SkillBarDialog/MiniMapDialog 同策略）；HeroAutoPotPreview 的 Point.Add(int,int) →
    // Add(Point)（MirMath.Point 无 int,int 重载）。
    ...
    public sealed class HeroBeltDialog : MirImageControl
    {
        ...
        public HeroBeltDialog()
        {
            ...
            // 裁剪：原 Hint 拼接 CMain.InputKeys.GetKey(KeybindOptions.Belt)（Unity 无 KeybindOptions）→ 按键名空。
            CloseButton = new MirButton
            {
                HoverIndex = 1924,
                Index = 1923,
                ...
                PressedIndex = 1925,
                Sound = SoundList.ButtonA,
                Hint = GameLanguage.ClientTextMap.GetLocalization(ClientTextKeys.CloseKey)
            };
            CloseButton.Click += (o, e) => Hide();
    ...
    public sealed class HeroMenuPanel : MirImageControl
    {
        ...
        public HeroMenuPanel(MirControl parent)
        {
            ...
            // 裁剪：3 个按钮 Hint 原拼接 CMain.InputKeys.GetKey(KeybindOptions.HeroSkills/HeroInventory/HeroEquipment)
            // （Unity 无 KeybindOptions）→ 按键名空，与 HeroBeltDialog 同策略。
            HeroMagicsButton = new MirButton
            {
                ...
                Hint = GameLanguage.ClientTextMap.GetLocalization(ClientTextKeys.HeroSkills)
            };
            ...
            HeroInventoryButton = new MirButton
            {
                ...
                Hint = GameLanguage.ClientTextMap.GetLocalization(ClientTextKeys.HeroInventory)
            };
            ...
            HeroEquipmentButton = new MirButton
            {
                ...
                Hint = GameLanguage.ClientTextMap.GetLocalization(ClientTextKeys.HeroCharacter)
            };
    ...
    public sealed class HeroInfoPanel : MirImageControl
    {
        ...
        // 探针只读访问（Avatar 为私有渲染组件）：返回 Update() 设置的当前头像图集索引。
    ...
    public sealed class HeroAutoPotPreview : MirImageControl
    {
        ...
        void AutoPotPreview_AfterDraw(object sender, EventArgs e)
        {
            ...
            Libraries.Items.Draw(Info.Image, DisplayLocation.Add(new Point(2, 3)), Size, Color.White);
            AmountLabel.Draw();
        }
