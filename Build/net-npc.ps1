# Iteration 3 orchestration: start server (Server.exe standalone) -> Unity npc probe -> assert -> stop server.
# Npc probe: login -> StartGame -> render real NPCDialog (dialogue pages/option buttons) + NPCGoodsDialog
#   (8-cell shop list) + StorageDialog (10x16 storage grid) control tree -> PNG + assertions.
param(
...
& $unity -batchmode -projectPath $proj -executeMethod Crystal.Rendering.Editor.NetProbe.RunNpc -logFile $log | Out-Null
$code = $LASTEXITCODE
...
$ok = ($code -eq 0) -and ($line -match "npc ok")
