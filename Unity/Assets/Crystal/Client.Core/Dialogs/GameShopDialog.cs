...
{
    // 逐字移植（2026-08-06）：Client/MirScenes/Dialogs/GameshopDialog.cs
    // 商城主窗：8 格商品区（4×2）+ 分类筛选列表 + 搜索框 + 职业/栏目切换 + 金币/点券支付勾选 +
    // 分页 + 滚动位置条。数据源 GameScene.GameShopInfoList（服务器 GameShopInfo 包推送）。
    // 裁剪：SystemInformation.MouseWheelScrollDelta（WinForms，Unity 无）→ 固定 120。
