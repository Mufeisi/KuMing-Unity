...
{
    // 逐字移植（2026-08-06）：Client/MirScenes/Dialogs/MailDialogs.cs 邮件系统对话框控制树。
    // MailListDialog 邮件列表（Title 670，10 行 MailItemRow 分页 + 发/回/读/删/黑名单/Bug 按钮）
    ...
    public class MailListDialog : MirImageControl
    {
        ...
        public MailListDialog()
        {
            ...
            // DeleteButton.Click 原 MirMessageBox YesNo 确认删除（未移植弹窗基类）→ 改为直接发包。
