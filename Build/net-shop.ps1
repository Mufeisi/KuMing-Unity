# Iteration 9 orchestration: start server (Server.exe standalone) -> Unity shop/socket/compass/report probe -> assert -> stop server.
# Shop probe: login -> StartGame -> synthesize GameShop/SocketDialog/CompassDialog/ReportDialog test data -> render
#   GameShopDialog + GameShopCell + GameShopViewer + SocketDialog + CompassDialog + ReportDialog control tree -> PNG + assertions.
param(
...
& $unity -batchmode -projectPath $proj -executeMethod Crystal.Rendering.Editor.NetProbe.RunShop -logFile $log | Out-Null
$code = $LASTEXITCODE
...
$ok = ($code -eq 0) -and ($line -match "shop ok")
