...
{
    // 逐字移植（2026-08-06）：Client/MirControls/MirAnimatedControl.cs
    // 动画控件：Animated/AnimationCount/AnimationDelay 三要素驱动 OffSet 帧推进 +
    // FadeIn 淡入（Opacity 累加）+ Loop/AfterAnimation 收尾回调 + 全局 Animations 注册表。
    // 帧推进由外部每帧调用 UpdateOffSet 驱动（Unity 侧渲染循环接入）。
