# Iteration 2 orchestration: start server (Server.exe standalone) -> Unity bag probe -> assert -> stop server.
# Bag probe: login -> StartGame -> render real InventoryDialog (bag grid + weight bar) + CharacterDialog
#   (14 equipment slots + character sprite) + item tooltip (CreateItemLabel) control tree -> PNG + assertions.
param(
...
& $unity -batchmode -projectPath $proj -executeMethod Crystal.Rendering.Editor.NetProbe.RunInventory -logFile $log | Out-Null
$code = $LASTEXITCODE
...
$ok = ($code -eq 0) -and ($line -match "bag ok")
