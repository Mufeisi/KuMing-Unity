# Iteration 5 orchestration: start server (Server.exe standalone) -> Unity quest probe -> assert -> stop server.
# Quest probe: login -> StartGame -> render real QuestList/QuestDiary/QuestDetail/QuestTracking + BigMap/MiniMap
#   control tree (task list/diary group/tracking bar/big map viewport/mini map viewport) -> PNG + assertions.
param(
...
& $unity -batchmode -projectPath $proj -executeMethod Crystal.Rendering.Editor.NetProbe.RunQuest -logFile $log | Out-Null
$code = $LASTEXITCODE
...
$ok = ($code -eq 0) -and ($line -match "quest ok")
