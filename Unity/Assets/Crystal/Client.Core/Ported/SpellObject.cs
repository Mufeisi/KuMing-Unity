...
{
    // 逐字移植（2026-08-08）：sanduan 提取 A1 SpellObject.cs（378 行，源参照
    // sanduan/Crystal-master/Client/MirObjects/SpellObject.cs）。魔法投射物对象：
    // Load(S.ObjectSpell) 按 Spell 枚举选帧（Magic/Magic2/Magic3/Effect/Dragon/Monsters 库 +
    // DrawFrame/FrameInterval/FrameCount/Repeat/Blend/Light）+ 地图特效/音效副作用；
    // Process 帧推进（Repeat 循环/单次 Ended）+ DrawLocation 世界→屏幕换算；
    // Draw 混合/常规二分（Blend→DrawBlend 0.8F）。Monster 型 BOSS 法术（TucsonGeneralRock/
    // HornedSorcererDustTornado/HornedCommanderRockSpike）Process 内声音/末帧定格特例逐字保留。
    public class SpellObject : MapObject
    {
        ...
        public Spell Spell;
        public Point AnimationOffset = new Point(0, 0);
        ...
        public void Load(S.ObjectSpell info)
        {
            ...
            switch (Spell)
            {
                ...
                case Spell.DigOutZombie:
                    BodyLibrary = (ushort)Monster.DigOutZombie < Libraries.Monsters.Length ? Libraries.Monsters[(ushort)Monster.DigOutZombie] : Libraries.Magic;
                ...
                case Spell.ExplosiveTrap:
                    ...
                    if (info.Param)
                    {
                        ...
                        FrameCount = 9;
                        Repeat = false;
                        SoundManager.PlaySound(20000 + 124 * 10 + 5); // 范围内玩家全员爆炸音效
                    }
                    else
                    {
                        DrawFrame = 1560;
