...
{
    // 逐字移植（2026-08-06）：Client/MirScenes/Dialogs/QuestDialogs.cs 任务系统对话框控制树。
    // QuestListDialog 任务列表（5 行 QuestRow + 接受/完成/离开 + QuestMessage 富文本 + QuestRewards 奖励）
    // / QuestDetailDialog 任务详情 / QuestDiaryDialog 任务日志（GroupBy 分组）/ QuestTrackingDialog 任务追踪
    // / QuestMessage 富文本消息（{文本/颜色} C 正则 + 滚动）/ QuestRewards 奖励 / QuestCell 物品格
    ...
    public sealed class QuestListDialog : MirImageControl
    {
        ...
        public QuestListDialog()
        {
            ...
            _finishButton.Click += (o, e) =>
            {
                ...
                {
                    // 原 MirMessageBox 弹窗"必须选择奖励物品"（未移植弹窗基类）→ 直接返回。
                    return;
                }
            ...
            // helpButton.Click 原打开 GameScene.Scene.HelpDialog.DisplayPage（HelpDialog 未移植）→ 裁剪。
        ...
        public bool GetAvailableQuests()
        {
            ...
            if (npc != null)
            {
                // 原 npc.GetAvailableQuests()（未移植方法）：Unity NPCObject.Quests 为 List<ClientQuestInfo>，
                // 由此构造可显示的任务进度对象（裁剪完成/进度状态，探针可控）。
                Quests = new List<ClientQuestProgress>();
                foreach (ClientQuestInfo info in npc.Quests)
                    Quests.Add(new ClientQuestProgress { Id = info.Index, QuestInfo = info });
            }
    ...
    public sealed class QuestDetailDialog : MirImageControl
    {
        ...
        public QuestDetailDialog()
        {
            ...
            {
                // 原 MirMessageBox YesNo 确认"放弃任务"（未移植弹窗基类）→ 直接放弃并关闭。
    ...
    public sealed class QuestMessage : MirControl
    {
        ...
        private void NewText(List<string> lines, bool resetIndex = true)
        {
            ...
            for (int i = TopLine; i < BottomLine; i++)
            {
                ...
                // 裁剪 NPCDialog 链接静态（MonsterLink/NPCLink/ItemLink + 4 方法未移植），仅保留 C 正则彩色文本。
        ...
        {
            // 旧客户端用 Color.FromName(colour)；MirMath.Color 无命名色，统一黄（NPCDialog 先例）。
