# Iteration 7 orchestration: start server (Server.exe standalone) -> Unity market probe -> assert -> stop server.
# Market probe: login -> StartGame -> render TradeDialog/GuestTradeDialog + 5 mail dialogs +
#   TrustMerchantDialog (Market + Consign panels) -> PNG + assertions.
param(
...
& $unity -batchmode -projectPath $proj -executeMethod Crystal.Rendering.Editor.NetProbe.RunMarket -logFile $log | Out-Null
$code = $LASTEXITCODE
...
$ok = ($code -eq 0) -and ($line -match "market ok")
