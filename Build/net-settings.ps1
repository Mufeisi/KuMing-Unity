# Iteration 10 orchestration: start server (Server.exe standalone) -> Unity settings probe -> assert -> stop server.
# Settings probe: login -> StartGame -> synthesize ChatOptionDialog/HelpDialog/KeyboardLayoutDialog test data -> render
#   control trees (filter tab / transparency tab / help page / keybind rows) -> PNGs + assertions.
param(
...
& $unity -batchmode -projectPath $proj -executeMethod Crystal.Rendering.Editor.NetProbe.RunSettings -logFile $log | Out-Null
$code = $LASTEXITCODE
...
$ok = ($code -eq 0) -and ($line -match "settings ok")
