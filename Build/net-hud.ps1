# P4-M5 orchestration: start server (Server.exe standalone) -> Unity HUD probe -> assert -> stop server.
# HUD probe: login -> StartGame -> real-server objects drive scene (R11) -> S.UserInformation-driven
#   HP/MP/Level status bars overlaid (R8 text pipeline) -> PNG + pixel assertions.
param(
...
& $unity -batchmode -projectPath $proj -executeMethod Crystal.Rendering.Editor.NetProbe.RunHud -logFile $log | Out-Null
$code = $LASTEXITCODE
...
$ok = ($code -eq 0) -and ($line -match "hud ok")
