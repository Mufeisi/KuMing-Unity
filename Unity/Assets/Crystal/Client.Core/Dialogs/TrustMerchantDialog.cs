...
{
    // 逐字移植（2026-08-06）：Client/MirScenes/Dialogs/TrustMerchantDialog.cs 市场/寄售/拍卖/商城
    // 四面板对话框控制树（Title 786/787，AuctionRow 10 行 + Filter 分类树 + 滚动条 + 搜索）。
    // BuyButton.Click 原弹 MirMessageBox 确认购买/领回 + MirAmountBox 输入出价（未移植弹窗基类）
    // → 确认类分支改直发包，Auction 出价分支（依赖 MirAmountBox 取 BidPrice）点击留空；
    // SearchTextBox_KeyPress 的 Program.Form.ActiveControl=null（窗体焦点无对应）→ 裁剪。
    // 网络发包（C.MarketPage/MarketRefresh/MarketSearch/MarketGetBack/MarketBuy/MarketSellNow/
    // ConsignItem）保留（Network 已可用）。
    public sealed class TrustMerchantDialog : MirImageControl
    {
        ...
        public TrustMerchantDialog()
        {
            ...
            BuyButton.Click += (o, e) =>
            {
                ...
                if (UserMode)
                {
                    // 原 "For Sale"/"No Bid" 未售出分支弹 MirMessageBox YesNo 确认领回（未移植）
                    // → 统一直发包（已售/过期分支本就直发包）。
                ...
                else
                {
                    switch (Selected.Listing.ItemType)
                    {
                        ...
                        case MarketItemType.GameShop:
                            // 原弹 MirMessageBox YesNo 确认购买（未移植）→ 直接发包。
                            MarketTime = CMain.Time + 3000;
