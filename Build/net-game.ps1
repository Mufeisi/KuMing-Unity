# P4-M3 orchestration: start server (Server.exe standalone) -> Unity game probe -> assert -> stop server.
# Game probe: login -> create char if none -> StartGame -> real-server object packets drive MapObject
#   (ObjectMonster/ObjectNPC spawn, ObjectTurn/Walk movement) -> R11 rendering -> assert.
param(
...
& $unity -batchmode -projectPath $proj -executeMethod Crystal.Rendering.Editor.NetProbe.RunGame -logFile $log | Out-Null
$code = $LASTEXITCODE
...
$ok = ($code -eq 0) -and ($line -match "game ok")
