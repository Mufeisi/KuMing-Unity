# Iteration 8 orchestration: start server (Server.exe standalone) -> Unity hero/mount probe -> assert -> stop server.
# Hero/mount probe: login -> StartGame -> synthesize UserHeroObject/Mount test data -> render
#   HeroInventoryDialog/HeroBeltDialog/HeroInfoPanel/HeroBehaviourPanel/HeroManageDialog/
#   MountDialog/HeroMenuPanel control tree -> PNG + assertions.
param(
...
& $unity -batchmode -projectPath $proj -executeMethod Crystal.Rendering.Editor.NetProbe.RunHero -logFile $log | Out-Null
$code = $LASTEXITCODE
...
$ok = ($code -eq 0) -and ($line -match "hero ok")
