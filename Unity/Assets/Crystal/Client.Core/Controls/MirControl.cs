...
{
    // 逐字移植（2026-08-05）：Client/MirControls/MirControl.cs
    // 去 WinForms/SlimDX/D3D9：控件不再拥有 DX render-target 纹理，Draw 管线直接下发
    // MLibrary seam（→ MLibraryUnity → CrystalSpriteBatch）与 TextRenderer seam（→ 动态字体）绘制。
    // 剪掉的平台面：ControlTexture/CreateTexture/DisposeTexture/DXManager、Border 折线绘制、
    // BlendMode 属性（HUD/Chat 不引用，Shared 与 MirMath 同名枚举值不同，规避歧义直接剔除）、
    // System.Drawing.Font 缩放（DPI 由渲染层处理，ScaleFont 原样返回）。
    public class MirControl : IDisposable
    {
        ...
        // 旧客户端的 per-control DX render-target 已移除（控件直接经 seam 绘制）。
        // 保留标志位以满足 MirLabel/MirTextBox 的旧 ctor 语义（文本非空才绘制）。
        public bool DrawControlTexture { get; set; }
        #endregion
        ...
        {
            // 旧基类 DrawControl：DrawControlTexture=true 时以 BackColour 清空 offscreen RT 再整幅绘制。
            // 纯 C# 端口无 RT，等价语义 = 直接填实心 BackColour 矩形（透明即不填），
            // 文本型子类（MirLabel/MirTextBox）在 override 中追加字形绘制。
            if (!DrawControlTexture)
                return;
            if (BackColour.A > 0)
                TextRenderer.FillBackground(this, new Rectangle(DisplayLocation, Size), BackColour);
        }
        ...
        {
            // 旧客户端用 D3D9 Line 画调试边框；纯 C# 端口无此路径，Border=false 默认不画。
        }
        ...
        #region Font
        // 旧客户端按 GDI+ DPI 缩放 System.Drawing.Font；Unity 动态字体尺寸由渲染层直接按 pt 处理，
        // ScaleFont 原样返回（DPI=96 假定，WinForms 平台面已剥离）。
