...
| 阶段7 Android Host | `Unity/Assets/Crystal/Client.Rendering.Editor/BuildAndroid.cs` + `Assets/Scenes/Main.unity` + ProjectSettings（Android/iOS：company/product/bundle/minSdk/目标版本/图标） | ✅ APK 构建通过（`[build-android] OK`，Unity/Build/Android/crystal.apk 16MB，产物不入库）；G7 门禁（触控/移动资源包/摇杆/简化 HUD）待落地 | 阶段7 |
