...
namespace Client.MirGraphics
{
    ...
    public class ParticleEngine
    {
            ...
            switch (type)
            {
                ...
                case ParticleType.Rain:
                    particle = new Particle()
                    {
                        Engine = this,
                        ImageInfo = tex,
                        Color = new Color(0x85, 0xFF, 0xFF, 0xFF),
                        // Color = Color.White,
                        Size = 1F,
                        BlendRate = 1F,
                        AliveTime = DateTime.MaxValue,
        ...
        // 探针支持：当前存活粒子数（验证生成/消亡节律）。非移植语义。
        public int ParticleCount { get { return particles == null ? 0 : particles.Count; } }

        // 探针支持：存活粒子列表只读访问（验证位移/回卷/绘制帧）。非移植语义。
