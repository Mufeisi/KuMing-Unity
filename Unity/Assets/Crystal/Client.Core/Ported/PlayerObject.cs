...
namespace Client.MirObjects
{
    public class PlayerObject : MapObject
    {
        ...
        public void FishingUpdate(S.FishingUpdate p)
        {
            ...
            {
                MirDirection dir = Functions.DirectionFromPoint(CurrentLocation, FishingPoint);
                        ...
                        switch (Spell)
                        {
                            ...
                            case Spell.ExplosiveTrap:
                                ...
                                if (this == User)
                                {
                                    ...
                                    Network.Enqueue(new C.Magic { ObjectID = GameScene.User.ObjectID, Spell = Spell, Direction = Direction, TargetID = targetID, Location = new SDPoint(location.X, location.Y) });
                                    MapControl.NextAction = CMain.Time + 1000;
                                    GameScene.SpellTime = CMain.Time + 1500; //Spell Delay
                                }
                                break;
                        ...
                        case MirAction.AttackRange1:
                            {
                                ...
                                Network.Enqueue(new C.RangeAttack { Direction = Direction, Location = new SDPoint(CurrentLocation.X, CurrentLocation.Y), TargetID = targetID, TargetLocation = new SDPoint(location.X, location.Y) });
                            }
                            break;
                        case MirAction.AttackRange2:
                        case MirAction.Spell:
