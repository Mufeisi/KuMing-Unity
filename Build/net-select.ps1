# P4-M2 orchestration: start server (Server.exe standalone) -> Unity select+enter-game probe -> assert -> stop server.
# Chain: login -> create char if none -> StartGame -> assert S.StartGame(4)+MapInformation+UserInformation.
param(
...
& $unity -batchmode -projectPath $proj -executeMethod Crystal.Rendering.Editor.NetProbe.RunSelect -logFile $log | Out-Null
$code = $LASTEXITCODE
...
$ok = ($code -eq 0) -and ($line -match "select ok")
