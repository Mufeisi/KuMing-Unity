...
{
    // 逐字移植（2026-08-06）：Client/MirScenes/Dialogs/BuffDialog.cs 渲染核心
    // Buff 状态栏：BuffIcon/MagIcon/Prguse2 图标 + 展开/折叠按钮 + 图标数标签 + 淡入淡出 + 冷却闪烁。
    // GetExpandedParameter/SetExpandedParameter 委托：Unity 无 GameScene 构造注入入口，
    // 构造时绑定 Settings.ExpandedBuffWindow（探针可改 Settings 控制展开态）。
    // 裁剪：BuffType.Guild case（依赖 GuildDialog.ActiveStats 未移植）；PoisonBuffDialog（未完成类，
    // GameScene 未引用，属后续迭代）。
    public class BuffDialog : MirImageControl
    {
        ...
        public BuffDialog()
        {
            ...
            SetExpandedParameter = v => Settings.ExpandedBuffWindow = v;
