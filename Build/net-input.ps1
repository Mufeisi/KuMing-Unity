# Iteration 2 input orchestration: start server (Server.exe standalone) -> Unity input probe -> assert -> stop server.
# Input probe: login -> StartGame -> synthesize mouse events over real MainDialog/ChatDialog/InventoryDialog
#   control tree (hover -> pressed -> click state transitions + ChatTextBox caret) -> PNG + assertions.
param(
...
& $unity -batchmode -projectPath $proj -executeMethod Crystal.Rendering.Editor.NetProbe.RunUiInput -logFile $log | Out-Null
$code = $LASTEXITCODE
...
$ok = ($code -eq 0) -and ($line -match "input ok")
