...
{
    // 逐字移植（2026-08-05）：Client/MirScenes/Dialogs/CharacterDialog.cs 渲染核心
    // 装备窗口（迭代包2）。14 槽位 MirItemCell（ItemSlot=EquipmentSlot 值）+ CharacterPage.AfterDraw
    // 角色立绘（StateItems 盔甲/武器 + Prguse 发型）+ Status/State 页数据标签 + 页签切换逐字保留。
    // 技能页（迭代包4）：7 格 MagicButton（Location=(8,8+i*33)）+ NextButton/BackButton 翻页
    // + RefreshInterface 按 StartIndex 分页填充 Actor.Magics。
    // 裁剪（属后续迭代）：Hide 里 SocketDialog 引用（未移植）、装备交互（穿脱网络包）。
