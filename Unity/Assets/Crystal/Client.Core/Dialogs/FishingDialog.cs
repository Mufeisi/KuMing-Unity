...
{
    // 逐字移植（2026-08-06）：Client/MirScenes/Dialogs/FishingDialog.cs
    // 钓鱼窗口（阶段6 补验剩余项）。FishingDialog 渔具槽（MirGridType.Fishing 5 格：
    // Hook/Float/Bait/Finder/Reel）+ 鱼竿名/鱼竿图绘制；FishingStatusDialog 施放状态条
    // （ChanceBar/ProgressBar Prguse 1342/1349 按 2.16*percent 绘制）+ FishButton 动画按钮
    // （Index=170 帧 AnimationCount=10 Loop，按到发 C.FishingCast{CastOut=false}）。
    // 差异：FishButton 声明为 MirAnimatedButton（源为 MirButton 字段赋动画按钮实例），
    // 匹配 PlayerObject.FishingUpdate 的显式 cast 契约。
    ...
    public sealed class FishingStatusDialog : MirImageControl
    {
        ...
        public MirButton CloseButton, AutoCastButton, ESCExitButton;
        public MirAnimatedButton FishButton;
