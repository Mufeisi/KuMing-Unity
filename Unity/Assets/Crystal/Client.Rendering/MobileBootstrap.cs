...
namespace Crystal.Client.Rendering
{
    ...
    public sealed class MobileBootstrap : MonoBehaviour
    {
        ...
        readonly MobileBag _settings = new MobileBag(1280, 720); // 设置按钮（8-8-4）：左缘钓鱼按钮下方开/关设置（ChatOptionDialog，灰 tint）
        ...
        void Start()
        {
            ...
            // 开/关英雄菜单（HeroMenuPanel：技能/背包/装备三按钮）。右列已满（第 10 列 720 高即超屏）
            // → 左缘顶部锚定（LeftAnchored，x=90 距左缘、y=100 距顶，避开顶部血条与底部聊天按钮）。
            _hero.OnToggle = ToggleHero;
            _hero.LeftAnchored = true;
            _hero.SetMargin(new Vector2(90f, 100f));
            _hero.TintOpen = new Color(1f, 1f, 1f, 0.95f);
            _hero.TintClosed = new Color(0.75f, 0.75f, 0.75f, 0.95f);

            // 开/关坐骑面板（MountDialog：骑乘/装备展示/忠诚度）。左缘英雄按钮下方（x=90，
            // y=英雄 100 + ButtonH 54 + 8 间距 = 162），避开顶部血条与底部聊天按钮。
            _mount.OnToggle = ToggleMount;
            _mount.LeftAnchored = true;
            ...
            // 开/关设置（ChatOptionDialog：聊天筛选+透明，主设置页）。左缘钓鱼按钮下方（x=90，
            // y=钓鱼 224 + ButtonH 54 + 8 间距 = 286），距底部聊天按钮区（y≥560）220px 余量。
            _settings.OnToggle = ToggleSettings;
            _settings.LeftAnchored = true;
            _settings.SetMargin(new Vector2(90f, 100f + (MobileBag.ButtonH + 8f) * 3));
            _settings.TintOpen = new Color(0.85f, 0.85f, 0.85f, 0.95f);
            _settings.TintClosed = new Color(0.55f, 0.55f, 0.55f, 0.95f);
            ...
            MobileUiAdapter.BackHandler = () =>
            {
                ...
                // 英雄（8-8-1）：HeroMenuPanel 优先（入口弹层），其次 HeroManageDialog/HeroInventoryDialog。
                var heroMenu = scene != null ? scene.HeroMenuPanel : null;
                if (heroMenu != null && heroMenu.Visible) { heroMenu.Visible = false; return true; }
                var heroInv = scene != null ? scene.HeroInventoryDialog : null;
                if (heroInv != null && heroInv.Visible) { heroInv.Hide(); return true; }
                var heroManage = scene != null ? scene.HeroManageDialog : null;
                if (heroManage != null && heroManage.Visible) { heroManage.Hide(); return true; }
                ...
                // 设置（8-8-4）：ChatOptionDialog 独立顶层窗，Back 在钓鱼之后关闭。
                var optionDlg = scene != null ? scene.ChatOptionDialog : null;
                if (optionDlg != null && optionDlg.Visible) { optionDlg.Hide(); return true; }
        ...
        void RenderHud()
        {
            ...
            if (_hero.ScreenW != GameRuntime.ScreenW || _hero.ScreenH != GameRuntime.ScreenH)
            ...
            if (_settings.ScreenW != GameRuntime.ScreenW || _settings.ScreenH != GameRuntime.ScreenH)
                _settings.SetScreen(GameRuntime.ScreenW, GameRuntime.ScreenH);
            ...
            var chatOption = scene != null ? scene.ChatOptionDialog : null;
            var keyLayout = scene != null ? scene.KeyboardLayoutDialog : null;
            var help = scene != null ? scene.HelpDialog : null;
            ...
            if (heroMenu != null && heroMenu.Visible) UiText.WarmTree(heroMenu); // 英雄（8-8-1）：菜单三按钮 Hint
            ...
            if (chatOption != null && chatOption.Visible) UiText.WarmTree(chatOption); // 设置（8-8-4）：筛选/透明按钮
            if (keyLayout != null && keyLayout.Visible) UiText.WarmTree(keyLayout);
            if (help != null && help.Visible) UiText.WarmTree(help);
            ...
            if (heroMenu != null && heroMenu.Visible) heroMenu.Draw();
            ...
            if (chatOption != null && chatOption.Visible) chatOption.Draw();
            if (keyLayout != null && keyLayout.Visible) keyLayout.Draw();
            if (help != null && help.Visible) help.Draw();
        ...
        void PollJoystick()
        {
            ...
            for (int i = 0; i < Input.touchCount; i++)
            {
                ...
                {
                    UiConsumer = (id, ph, ui) => _bag.OnTouch(id, ph, ui) || _equip.OnTouch(id, ph, ui) || _quest.OnTouch(id, ph, ui) || _map.OnTouch(id, ph, ui) || _chat.OnTouch(id, ph, ui) || _group.OnTouch(id, ph, ui) || _friend.OnTouch(id, ph, ui) || _guild.OnTouch(id, ph, ui) || _mail.OnTouch(id, ph, ui) || _market.OnTouch(id, ph, ui) || _gameShop.OnTouch(id, ph, ui) || _hero.OnTouch(id, ph, ui) || _mount.OnTouch(id, ph, ui) || _fishing.OnTouch(id, ph, ui) || _settings.OnTouch(id, ph, ui), // 背包/装备/任务/地图/聊天/组队/好友/行会/邮件/拍卖行/商城/英雄/坐骑/钓鱼/设置按钮（ui 空间，短路：背包先消费）
                    PanelOpen = bagOpen,
        ...
        // 英雄菜单开/关（8-8-1）：切换 HeroMenuPanel.Visible（入口弹层：技能/背包/装备三按钮，
        // 旧版挂 MainDialog 由 HeroMenuButton 开，移动端用左缘按钮）。无英雄（Hero null）时不弹
        // （服务器先推 S.HeroInformation 才有英雄）。开→互斥关背包/装备/组队/好友/行会/邮件/
        // 拍卖行/商城面板 + Cancel 摇杆/HUD/拾取/寻路。关→仅藏菜单（子面板由 Back/按钮各自关）。
        void ToggleHero(bool open)
        {
            var scene = GameScene.Scene;
            var menu = scene != null ? scene.HeroMenuPanel : null;
            if (menu == null) return;
            try
            {
                if (open)
                {
                    if (GameScene.Hero == null) return; // 无英雄不弹（S.HeroInformation 未到/未创建英雄）
                    if (!menu.Visible)
                    {
                        var inv = scene != null ? scene.InventoryDialog : null;
                        if (inv != null && inv.Visible) inv.Hide();
            ...
            {
                Debug.LogError($"[mobile] hero-toggle {ex.GetType().Name}: {ex.Message}");
            }
            Debug.Log($"[mobile] hero-{(open ? "open" : "close")} visible={menu.Visible}");
        }

        // 坐骑面板开/关（8-8-2）：切换 MountDialog.Visible。开走 Show()——无坐骑（User.MountType<0）
        // 弹 NoMount 提示不打开（旧版 Show 语义）；有坐骑显示面板 + 互斥关背包/装备/组队/好友/行会/
        // 邮件/拍卖行/商城/英雄菜单 + Cancel 摇杆/HUD/拾取/寻路。关只 Hide。
        void ToggleMount(bool open)
        {
            var scene = GameScene.Scene;
            var mount = scene != null ? scene.MountDialog : null;
        ...
        // 设置开/关（8-8-4）：切换 ChatOptionDialog.Visible（聊天筛选+透明，主设置页）。开→
        // 互斥关背包/装备/组队/好友/行会/邮件/拍卖行/商城/英雄菜单/坐骑/渔具窗 + Cancel 摇杆/
        // HUD/拾取/寻路。关只 Hide。键位（KeyboardLayoutDialog）与帮助（HelpDialog）常驻由
        // 后续虚拟键面板任务接入口（backlog）。
        void ToggleSettings(bool open)
        {
            var scene = GameScene.Scene;
            var option = scene != null ? scene.ChatOptionDialog : null;
            ...
            try
            {
                if (open)
                {
                    if (!option.Visible)
                    {
                        ...
                        if (mount != null && mount.Visible) mount.Hide();
                        var fishing = scene != null ? scene.FishingDialog : null;
                        if (fishing != null && fishing.Visible) fishing.Hide();
                        option.Show();
                        _joystick.Cancel();
                        _hud.Cancel();
                        _pickup.Cancel();
            ...
            {
                Debug.LogError($"[mobile] settings-toggle {ex.GetType().Name}: {ex.Message}");
            }
            Debug.Log($"[mobile] settings-{(open ? "open" : "close")} visible={option.Visible}");
        }
