...
// manifest <dir> --out <file.json>：递归扫 dir 下所有文件，生成资源分发清单
// （每项 rel/size/sha256 + 汇总 count/totalBytes）。阶段7 第 4 项"版本校验"数据源：
// 客户端本地索引与之 PlanDiff，得需下载文件列表（缺失或 size/sha256 不一致）。
// 确定性：按 rel OrdinalIgnoreCase 排序，重复运行字节级一致。
static class Manifest
{
    internal sealed class Entry { public string Rel { get; set; } public long Size { get; set; } public string Sha256 { get; set; } }
    internal sealed class Output { public int Format { get; set; } public string GeneratedUtc { get; set; } public int Count { get; set; } public long TotalBytes { get; set; } public List<Entry> Files { get; set; } }
    ...
    internal static int Run(string[] args)
    {
        ...
        Console.WriteLine($"manifest dir={dir} files={entries.Count} bytes={total} -> {outAbs}");
        return 0;
    }
